...
function handleGetFileBase64(payload) {
  var fileId = payload.fileId || payload.FileId || "";
  var fileUrl = payload.fileUrl || payload.FileUrl || "";
  ...
  if (!fileId) {
    return createJsonResponse({ success: false, error: "fileId is required to fetch file binary." });
  }
  ...
  try {
    ...
    return createJsonResponse({
      success: false,
      error: "Error reading Drive file: " + err.toString()
    });
  }
}
