...
function doGet(e) {
  initAllSheets();
  if (e && e.parameter && (e.parameter.action === "getFileBase64" || e.parameter.fileId)) {
    return handleGetFileBase64(e.parameter);
  }
  ...
  try {
    ...
    switch (action) {
      ...
      case "getSubmissionOverrides":
        return handleGetSubmissionOverrides(payload);

      case "setSubmissionOverride":
        return handleSetSubmissionOverride(payload);

      case "deleteSubmissionOverride":
        return handleDeleteSubmissionOverride(payload);

      case "getFileBase64":
        return handleGetFileBase64(payload);
...
function initAllSheets() {
  getOrCreateSheet("Users", 
    ...
    [
      ["TCH-801", "admin123", "teacher", "Faculty Evaluator"]
    ]
  );
...
function handleGetFileBase64(payload) {
  var fileId = (payload.fileId || payload.FileId || "").trim();
  var fileUrl = (payload.fileUrl || payload.FileUrl || "").trim();
  ...
  if (!fileId && fileUrl) {
    var match = fileUrl.match(/(?:drive\.google\.com\/(?:file\/d\/|open\?id=|uc\?.*?id=)|docs\.google\.com\/(?:file\/d\/))([a-zA-Z0-9_-]{15,})/);
    if (match && match[1]) {
      fileId = match[1];
    }
  }

  // If fileId not supplied directly, locate in Submissions sheet
  if (!fileId && examId && studentId) {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var subSheet = ss.getSheetByName("Submissions");
    if (subSheet && subSheet.getLastRow() > 1) {
      var data = subSheet.getRange(2, 1, subSheet.getLastRow() - 1, 6).getValues();
      for (var i = 0; i < data.length; i++) {
        var rowStudentId = String(data[i][0]).trim().toUpperCase();
        var rowExamId = String(data[i][1]).trim().toUpperCase();
        if (rowStudentId === studentId.toUpperCase() && rowExamId === examId.toUpperCase()) {
          var targetUrl = (type === "graded" && data[i][4]) ? String(data[i][4]).trim() : String(data[i][2]).trim();
          var targetMatch = targetUrl.match(/(?:drive\.google\.com\/(?:file\/d\/|open\?id=|uc\?.*?id=)|docs\.google\.com\/(?:file\/d\/))([a-zA-Z0-9_-]{15,})/);
          if (targetMatch && targetMatch[1]) {
            fileId = targetMatch[1];
            break;
          }
        }
      }
  ...
  if (!fileId) {
    return createJsonResponse({
      success: false,
      error: "fileId is required to fetch file binary."
    });
  }
  ...
  try {
    ...
    var fullDataUri = "data:" + mimeType + ";base64," + base64;
    ...
    return createJsonResponse({
      success: false,
      error: "Error retrieving file from Google Drive: " + err.toString()
    });
  }
}
...
function handleSubmitAnswerSheet(payload) {
  ...
  if (examSheet) {
    ...
    for (var e = 1; e < examData.length; e++) {
      if (String(examData[e][0]).trim() === examId) {
        var rawEndTime = examData[e][2];
        if (rawEndTime) {
          var endTimestamp = new Date(rawEndTime).getTime();
          // Allow small 60-second grace window for upload network transit
          if (Date.now() > endTimestamp + 60000) {
            return createJsonResponse({
              success: false,
              error: "Submission rejected: Examination time is up. The submission window is closed."
            });
          }
        }
        break;
...
function handleGetSubmissionOverrides(payload) {
  var examId = (payload.examId || "").trim();
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = getOrCreateSheet(
    "SubmissionOverrides",
    ["OverrideId", "ExamId", "TargetType", "TargetId", "TargetName", "AllowSubmission", "Reason", "ExpiresAt", "GrantedBy", "CreatedAt"],
    []
  );
  ...
  for (var i = 1; i < data.length; i++) {
    var row = data[i];
    var rowExamId = String(row[1]).trim();
    if (!examId || rowExamId === examId) {
      overrides.push({
        OverrideId: String(row[0]),
        ExamId: rowExamId,
        TargetType: String(row[2]),
        TargetId: String(row[3]),
        TargetName: String(row[4] || row[3]),
        AllowSubmission: String(row[5]).toLowerCase() === "true" || row[5] === true,
        Reason: String(row[6] || ""),
        ExpiresAt: String(row[7] || ""),
  ...
  return createJsonResponse({ success: true, overrides: overrides });
}

function handleSetSubmissionOverride(payload) {
  var examId = (payload.examId || "").trim();
  var targetType = (payload.targetType || "STUDENT").toUpperCase().trim();
  var targetId = (payload.targetId || "").trim();
  var targetName = (payload.targetName || targetId).trim();
  var allowSubmission = payload.allowSubmission !== false;
  var reason = (payload.reason || "").trim();
  var expiresAt = (payload.expiresAt || "").trim();
  var grantedBy = (payload.grantedBy || "Faculty Evaluator").trim();
  var createdAt = new Date().toISOString();

  if (!examId || !targetId) {
    return createJsonResponse({ success: false, error: "ExamId and TargetId are required." });
  }
  ...
  var data = sheet.getDataRange().getValues();
  var existingRow = -1;
  var overrideId = "OVR-" + Math.floor(100000 + Math.random() * 900000);

  for (var i = 1; i < data.length; i++) {
    if (String(data[i][1]).trim() === examId && String(data[i][2]).toUpperCase().trim() === targetType && String(data[i][3]).trim() === targetId) {
      existingRow = i + 1;
      overrideId = String(data[i][0]);
      break;
    }
  }

  if (existingRow > -1) {
    sheet.getRange(existingRow, 5).setValue(targetName);
    sheet.getRange(existingRow, 6).setValue(allowSubmission ? "TRUE" : "FALSE");
    sheet.getRange(existingRow, 7).setValue(reason);
    sheet.getRange(existingRow, 8).setValue(expiresAt);
    sheet.getRange(existingRow, 9).setValue(grantedBy);
  } else {
    sheet.appendRow([
      overrideId,
      examId,
  ...
  return createJsonResponse({
    success: true,
    message: "Submission permission set successfully.",
    override: {
...
function handleDeleteSubmissionOverride(payload) {
  var overrideId = (payload.overrideId || "").trim();
  var examId = (payload.examId || "").trim();
  var targetId = (payload.targetId || "").trim();

  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName("SubmissionOverrides");
  if (!sheet) {
    return createJsonResponse({ success: true, message: "No overrides sheet." });
  }
  ...
  for (var i = data.length - 1; i >= 1; i--) {
    var matchId = overrideId && String(data[i][0]).trim() === overrideId;
    var matchTarget = examId && targetId && String(data[i][1]).trim() === examId && String(data[i][3]).trim() === targetId;
  ...
  return createJsonResponse({
    success: true,
    message: deleted ? "Submission override deleted." : "Override not found."
  });
}
