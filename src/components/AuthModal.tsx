  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-xs p-4">
      <motion.div
        ...
        className="relative w-full max-w-md bg-white border border-sky-200/80 rounded-2xl flex flex-col shadow-2xl overflow-hidden font-sans"
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 bg-white">
          {/* Role Pill Switcher */}
          <div className="relative grid grid-cols-2 gap-2 p-1 bg-slate-100/90 rounded-xl border border-slate-200">
            <button
              type="button"
              onClick={() => handleRoleChange('student')}
              className={`relative py-2 text-xs font-bold rounded-lg transition-colors flex items-center justify-center gap-1.5 min-h-[38px] z-10 ${
                role === 'student' ? 'text-[#009fe3]' : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              {role === 'student' && (
                <motion.div
                  layoutId="authRoleActive"
                  className="absolute inset-0 bg-white rounded-lg shadow-xs border border-sky-200 -z-10"
            ...
            <button
              ...
              className={`relative py-2 text-xs font-bold rounded-lg transition-colors flex items-center justify-center gap-1.5 min-h-[38px] z-10 ${
                role === 'teacher' ? 'text-[#f25f22]' : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              {role === 'teacher' && (
                <motion.div
                  layoutId="authRoleActive"
                  className="absolute inset-0 bg-white rounded-lg shadow-xs border border-orange-200 -z-10"
                  transition={{ type: 'spring', stiffness: 450, damping: 35 }}
                />
              )}
          ...
          <AnimatePresence>
            {error && (
              <motion.div
                ...
                className="p-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-xs flex items-start gap-2 overflow-hidden"
              >
