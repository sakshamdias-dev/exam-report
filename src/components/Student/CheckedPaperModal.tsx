...
import { getGradedPaperStorage } from '../../utils/fileStorage';
import { 
  X, 
  FileCheck, 
  Eye, 
...
import { generateQuestionWiseMarkingPdfBlob } from '../../utils/osmPdfGenerator';
...
export const CheckedPaperModal: React.FC<CheckedPaperModalProps> = ({
  ...
  // Hydrate resolved GradedUrl from memory / IndexedDB if submission has pointer or missing
  const [resolvedGradedUrl, setResolvedGradedUrl] = useState<string | null>(
    submission.GradedUrl && !submission.GradedUrl.startsWith('stored://') ? submission.GradedUrl : null
  );

  useEffect(() => {
    let active = true;
    const fetchStored = async () => {
      let url = submission.GradedUrl;
      if (!url || url.startsWith('stored://')) {
        const stored = await getGradedPaperStorage(submission.ExamId, submission.StudentId);
        if (stored && active) {
          setResolvedGradedUrl(stored);
          return;
        }
      }
      if (active) {
        setResolvedGradedUrl(url || null);
      }
    };
    fetchStored();
    return () => {
  ...
  }, [submission.ExamId, submission.StudentId, submission.GradedUrl]);

  if (!isOpen) return null;

  const evaluationDoubts: EvaluationDoubt[] = submission.EvaluationDoubts || [];
  const hasGradedPdf = !!resolvedGradedUrl;
  ...
  // Extract question marks breakdown from submission / osmDraftData / exam
  let questionMarksList: { id: string; label: string; maxMarks: number; awardedMarks: number | ''; comment?: string }[] = [];
  let parsedFeedback = submission.Feedback || '';
  let parsedTotalAwarded = submission.Score !== undefined && submission.Score !== '' ? Number(submission.Score) : 0;

  if (submission.OsmDraftData) {
    try {
      const parsed = typeof submission.OsmDraftData === 'string' ? JSON.parse(submission.OsmDraftData) : submission.OsmDraftData;
      if (Array.isArray(parsed.questionMarks) && parsed.questionMarks.length > 0) {
        questionMarksList = parsed.questionMarks;
      }
      if (parsed.overallFeedback) parsedFeedback = parsed.overallFeedback;
      if (parsed.totalScore !== undefined && parsed.totalScore !== '') parsedTotalAwarded = Number(parsed.totalScore);
    } catch (e) {}
  } else if (Array.isArray((submission as any).QuestionScores)) {
    questionMarksList = (submission as any).QuestionScores;
  } else if (exam?.QuestionMarks && exam.QuestionMarks.length > 0) {
    questionMarksList = exam.QuestionMarks.map((qm) => {
      const awarded = submission.QuestionScores && typeof submission.QuestionScores === 'object'
        ? (submission.QuestionScores[qm.id] ?? submission.QuestionScores[qm.label] ?? '')
        : '';
      return {
        id: qm.id,
        label: qm.label,
        maxMarks: qm.maxMarks,
        awardedMarks: awarded !== '' ? Number(awarded) : '',
      };
    });
  } else if (submission.QuestionScores && typeof submission.QuestionScores === 'object') {
    questionMarksList = Object.entries(submission.QuestionScores).map(([key, val], idx) => ({
      id: `q_${idx + 1}`,
      label: key,
      maxMarks: 10,
      awardedMarks: Number(val),
    }));
  }

  // Check local storage draft or exam question marks cache if still not found
  if (questionMarksList.length === 0) {
    try {
      const draftRaw = localStorage.getItem(`osm_draft_${submission.ExamId}_${submission.StudentId}`);
      if (draftRaw) {
        const parsedDraft = JSON.parse(draftRaw);
        if (Array.isArray(parsedDraft.questionMarks) && parsedDraft.questionMarks.length > 0) {
          questionMarksList = parsedDraft.questionMarks;
        }
        if (parsedDraft.overallFeedback && !parsedFeedback) parsedFeedback = parsedDraft.overallFeedback;
        if (parsedDraft.totalScore !== undefined && !parsedTotalAwarded) parsedTotalAwarded = Number(parsedDraft.totalScore);
      }
      if (questionMarksList.length === 0) {
        const storedQm = localStorage.getItem(`exam_qm_${submission.ExamId.toUpperCase()}`) || localStorage.getItem(`exam_qm_${submission.ExamId}`);
        if (storedQm) {
          const parsed = JSON.parse(storedQm);
          if (Array.isArray(parsed) && parsed.length > 0) {
            questionMarksList = parsed.map((qm: any, idx: number) => ({
              id: qm.id || `q_${idx + 1}`,
              label: qm.label || `Q${idx + 1}`,
              maxMarks: Number(qm.maxMarks) || 10,
              awardedMarks: '',
            }));
          }
        }
  ...
  const handleDownloadQuestionWiseReport = async () => {
    try {
      setDownloadingReport(true);
      const totalMax = exam?.TotalMarks || 100;
      const sessionData = {
        examId: submission.ExamId,
        studentId: submission.StudentId,
        studentName: submission.StudentName || submission.StudentId,
        subject: exam?.Subject || 'Subject Examination',
        totalPages: 1,
        annotationsPerPage: {},
        questionMarks: questionMarksList.length > 0 ? questionMarksList : [
          { id: 'q1', label: 'Complete Answer Booklet', maxMarks: totalMax, awardedMarks: parsedTotalAwarded, comment: parsedFeedback || 'Evaluation' }
        ],
        overallFeedback: parsedFeedback || submission.Feedback || 'Evaluated via On-Screen Marking (OSM).',
        totalScore: parsedTotalAwarded,
        isCompleted: true,
        lastSavedAt: submission.SubmittedAt || new Date().toISOString(),
      };

      const blob = await generateQuestionWiseMarkingPdfBlob(sessionData);
      const blobUrl = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = blobUrl;
      link.download = `ExamFriendly_Report_${submission.ExamId}_${submission.StudentId}.pdf`;
      ...
      alert('Could not download Question-Wise Marking Report PDF.');
    } finally {
      setDownloadingReport(false);
    }
  };

  const handleDownloadGradedPaper = async () => {
    let targetUrl = resolvedGradedUrl || submission.GradedUrl;
    if (!targetUrl || targetUrl.startsWith('stored://')) {
      const stored = await getGradedPaperStorage(submission.ExamId, submission.StudentId);
      if (stored) targetUrl = stored;
    }
    if (!targetUrl) {
      alert('Evaluated paper is still synchronizing or not found.');
      return;
    }
    ...
    try {
      const link = document.createElement('a');
      link.href = targetUrl;
      link.download = `Evaluated_Paper_${submission.ExamId}_${submission.StudentId}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } catch (e: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 backdrop-blur-xs p-3 sm:p-4 animate-in fade-in duration-150">
      <div className="relative w-full max-w-4xl bg-white border border-slate-200 rounded-2xl flex flex-col shadow-2xl overflow-hidden max-h-[92vh] font-sans">
        ...
        {/* Score & Evaluation Summary Banner: Total Question marks, Received marks & Grand total */}
        {(() => {
          const totalQMarks = questionMarksList.reduce((acc, q) => acc + (Number(q.maxMarks) || 0), 0) || (exam?.TotalMarks || 100);
          const receivedMarks = submission.Score !== '' && submission.Score !== undefined ? submission.Score : parsedTotalAwarded;
          return (
            <div className="px-5 sm:px-6 py-3 bg-gradient-to-r from-sky-50/70 via-white to-amber-50/40 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-4 flex-wrap">
                <div>
                  <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider block">Total Question Marks</span>
                ...
                <div>
                  <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider block">Received Marks</span>
                  <div className="text-sm font-black text-emerald-700 font-mono flex items-center gap-1 mt-0.5">
                    <Award className="w-3.5 h-3.5 text-[#f25f22]" />
                    <span>{receivedMarks}</span>
                  </div>
                </div>
                ...
                <div>
                  <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider block">Grand Total</span>
                  <div className="text-sm font-extrabold text-slate-900 font-mono mt-0.5">
                    {receivedMarks} / {totalQMarks}
                  </div>
                </div>
                ...
                {submission.Feedback && (
                  <>
                    ...
                    <div className="hidden sm:block">
                      <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider block">Evaluator Feedback</span>
                      <p className="text-xs text-slate-800 font-medium italic line-clamp-1 max-w-xs mt-0.5">
                        "{submission.Feedback}"
                      </p>
                    </div>
                  </>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  type="button"
                  onClick={handleDownloadQuestionWiseReport}
                  disabled={downloadingReport}
                  className="px-3 py-1.5 bg-[#009fe3] hover:bg-[#0088c4] text-white font-bold text-xs rounded-lg transition-colors flex items-center gap-1.5 shadow-xs cursor-pointer disabled:opacity-50"
                  title="Download Question-Wise Evaluation Report PDF"
                >
                  <Download className="w-3.5 h-3.5" />
                  <span>{downloadingReport ? 'Generating...' : 'Question-Wise Report'}</span>
                </button>

                {resolvedGradedUrl ? (
                  <button
                    onClick={() => {
                      setFullscreenPdfUrl(resolvedGradedUrl!);
        ...
        <div className="p-5 sm:p-6 overflow-y-auto flex-1 bg-slate-50/50 space-y-4">
          ...
          {activeTab === 'paper' && (
            <div className="space-y-4">
              ...
              {/* TEACHER REMARKS & EVALUATION FEEDBACK CARD */}
              <div className="bg-white border border-sky-200 rounded-2xl p-4 sm:p-5 shadow-xs relative overflow-hidden space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <div className="p-1.5 rounded-lg bg-sky-50 text-[#009fe3]">
                  ...
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-sky-50 text-[#009fe3] border border-sky-100 font-bold">
                    Official Release
                  </span>
                </div>

                <div className="p-3.5 rounded-xl bg-slate-50/80 border border-slate-200/80 text-xs text-slate-800 leading-relaxed font-sans">
                  {parsedFeedback || submission.Feedback ? (
                    ...
                    <p className="text-slate-400 italic">
                      No general remarks provided. Evaluation completed via per-question marking.
                    </p>
                  )}
                </div>

                {/* Rechecking remarks if recheck was conducted */}
                {submission.RecheckRemarks && (
                  <div className="p-3 rounded-xl bg-emerald-50/80 border border-emerald-200 text-xs text-emerald-900 space-y-1">
                    <div className="flex items-center gap-1.5 font-bold text-emerald-800">
                      ...
                      <span>Faculty Rechecking Resolution Remark:</span>
                      {submission.RecheckResolvedAt && (
                        <span className="text-[10px] font-mono font-normal text-emerald-700 ml-auto">
                          {formatDateTime(submission.RecheckResolvedAt)}
                        </span>
                      )}
                    </div>
              ...
              {/* QUESTION-WISE MARKING SUMMARY TABLE */}
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 shadow-xs space-y-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <button
                      ...
                      onClick={handleDownloadQuestionWiseReport}
                      disabled={downloadingReport}
                      className="px-3 py-1.5 bg-[#009fe3] hover:bg-[#0088c4] text-white text-xs font-bold rounded-xl transition-all shadow-xs flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                      title="Download branded Exam Friendly Question-Wise PDF report"
                    >
                      {downloadingReport ? (
                        <>
                        ...
                        <>
                          <Download className="w-3.5 h-3.5" />
                          <span>Download Question-Wise PDF</span>
                        </>
                      )}
                    </button>
                  </div>
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200">
                          ...
                          <th className="py-2.5 px-3">Examiner Comment</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 font-sans">
                        {questionMarksList.map((q, idx) => (
                          <tr key={q.id || idx} className="hover:bg-slate-50/80 transition-colors">
                            <td className="py-2 px-3 font-mono text-slate-500 font-bold">{idx + 1}</td>
                            <td className="py-2 px-3 font-medium text-slate-800">{q.label || `Question ${idx + 1}`}</td>
                            <td className="py-2 px-3 text-right font-mono text-slate-600">{q.maxMarks}</td>
                            <td className="py-2 px-3 text-right font-mono font-bold text-emerald-700">
                              {q.awardedMarks !== '' && q.awardedMarks !== undefined ? q.awardedMarks : '—'}
                            </td>
                        ...
                        {/* Summary Total Row */}
                        <tr className="bg-sky-50/70 font-bold text-slate-900 border-t-2 border-sky-200">
                          <td colSpan={2} className="py-2.5 px-3 text-sky-950 uppercase tracking-wider text-[11px]">
                            Total Summary
                          </td>
                          <td className="py-2.5 px-3 text-right font-mono text-slate-700">
                            {exam?.TotalMarks || questionMarksList.reduce((acc, q) => acc + (q.maxMarks || 0), 0)}
                          </td>
                          <td className="py-2.5 px-3 text-right font-mono text-emerald-700 text-sm">
                            {parsedTotalAwarded}
                          </td>
                          <td className="py-2.5 px-3 text-slate-600 text-[11px]">
                            {parsedTotalAwarded >= (exam?.TotalMarks ? exam.TotalMarks * 0.4 : 40) ? 'Passed' : 'Evaluated'}
                          </td>
                        </tr>
                      </tbody>
                    </table>
              ...
              {hasGradedPdf ? (
                <div className="space-y-3">
                  ...
                  {/* Embedded PDF canvas preview */}
                  <div className="w-full h-[420px] bg-slate-900 rounded-xl overflow-hidden border border-slate-300 shadow-inner flex flex-col">
                    <PdfCanvasViewer
                      url={resolvedGradedUrl}
                      examId={submission.ExamId}
                      studentId={submission.StudentId}
                      title={`Evaluated Booklet - ${submission.ExamId}`}
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={handleDownloadGradedPaper}
                        disabled={downloadingBooklet}
                        className="px-3 py-1.5 rounded-lg border border-slate-300 bg-white text-slate-800 hover:bg-slate-50 text-xs font-bold flex items-center gap-1.5 transition-colors"
                      >
                        <Download className="w-3.5 h-3.5" />
                        <span>{downloadingBooklet ? 'Downloading...' : 'Download Booklet PDF'}</span>
                      </button>
                  ...
                  <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
                    ...
                    {questionMarksList.length > 0 && (
                      <button
                        onClick={handleDownloadQuestionWiseReport}
                        disabled={downloadingReport}
                        className="px-4 py-2 bg-[#009fe3] text-white rounded-xl text-xs font-bold inline-flex items-center gap-2 hover:bg-[#0088c4] transition-colors"
                      >
                        <Download className="w-4 h-4" />
                        <span>Download Question-Wise Marking PDF</span>
                      </button>
                    )}
                  </div>
                </div>
