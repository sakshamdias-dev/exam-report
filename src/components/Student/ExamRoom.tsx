...
import { executeGasAction, fileToBase64, fileToBase64WithProgress, GasProgress } from '../../services/api';
...
import { SubmissionWindowModal } from './SubmissionWindowModal';
import { ExamFriendlyLogo } from '../ExamFriendlyLogo';
import { getEmbeddablePdfUrl, base64ToBlobUrl } from '../../utils/pdfHelper';
import { getQuestionPaperStorage } from '../../utils/fileStorage';
import { playDoubtAlertSound, playStudentReplyBeepSound } from '../../utils/audioAlert';
import { 
  isMobileDevice, 
...
export const ExamRoom: React.FC<ExamRoomProps> = ({ exam, currentUser, onExit, onSubmitted }) => {
  ...
  const [activeNotification, setActiveNotification] = useState<{
    ...
    title: string;
    message: string;
    type: 'warning' | 'alert' | 'info' | 'proctor' | 'doubt_reply';
    timestamp: string;
    badge?: string;
    doubtQuestion?: string;
    doubtReplySnippet?: string;
    fullReply?: string;
  } | null>(null);
  const previousDoubtsRef = useRef<Doubt[] | null>(null);
  const notifiedReplyKeysRef = useRef<Set<string>>(new Set());
  ...
  const [fileReadStatus, setFileReadStatus] = useState<string>('Buffering PDF document...');
  const [fileReadLoaded, setFileReadLoaded] = useState<number>(0);
  const [isDraggingFile, setIsDraggingFile] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [submitProgress, setSubmitProgress] = useState<GasProgress>({
    loaded: 0,
    total: 0,
    percent: 0,
    stage: 'preparing',
  ...
  // 1. Calculate and count down time (exam start, end time, and dedicated submission window)
  const submissionWindowMinutes = Math.min(
    10,
    Math.max(
      1,
      Number(
        exam.SubmissionWindowMinutes ??
        (() => {
          try {
            const saved = localStorage.getItem(`exam_sub_window_${exam.ExamId}`);
  ...
  const [writingTimeLeftSeconds, setWritingTimeLeftSeconds] = useState<number>(0);
  const [submissionTimeLeftSeconds, setSubmissionTimeLeftSeconds] = useState<number>(0);
  const [isWritingOver, setIsWritingOver] = useState<boolean>(() => {
    return Date.now() >= new Date(exam.EndTime).getTime();
  });
  const [isSubmissionWindowActive, setIsSubmissionWindowActive] = useState<boolean>(() => {
    const now = Date.now();
    const end = new Date(exam.EndTime).getTime();
    const subEnd = end + submissionWindowMinutes * 60 * 1000;
    return now >= end && now < subEnd;
  });
  const [isSubmissionWindowExpired, setIsSubmissionWindowExpired] = useState<boolean>(() => {
    const now = Date.now();
    const end = new Date(exam.EndTime).getTime();
    const subEnd = end + submissionWindowMinutes * 60 * 1000;
    return now >= subEnd;
  ...
  // Dedicated Submission Window Modal (pops up automatically upon exam writing conclusion)
  const [isSubmissionModalOpen, setIsSubmissionModalOpen] = useState(false);
  const hasAutoTriggeredSubmissionRef = useRef(false);
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      if (now < start) {
        ...
        setWritingTimeLeftSeconds(diffEnd);
        setSubmissionTimeLeftSeconds(submissionWindowMinutes * 60);
        setIsWritingOver(false);
        setIsSubmissionWindowActive(false);
        setIsSubmissionWindowExpired(false);
        setExamEnded(false);
      } else if (now < end) {
        ...
        // Active Submission Grace Period Window
        setIsBeforeStart(false);
        setSecondsUntilStart(0);
        setTimeLeftSeconds(0);
        setWritingTimeLeftSeconds(0);
        const diffSub = Math.max(0, Math.floor((subEnd - now) / 1000));
        setSubmissionTimeLeftSeconds(diffSub);
        setIsWritingOver(true);
        setIsSubmissionWindowActive(true);
        setIsSubmissionWindowExpired(false);
        setExamEnded(false); // Proctor surveillance continues during submission

        // Automatically trigger pop up for answer sheet submission
        if (!hasAutoTriggeredSubmissionRef.current && !submitSuccess) {
          hasAutoTriggeredSubmissionRef.current = true;
          setIsSubmissionModalOpen(true);
          setMobileTab('submit');
          setPreviewPdfUrl((curr) => (curr ? null : curr));
        }
      } else {
        // Submission window elapsed & exam concluded
        setIsBeforeStart(false);
        setSecondsUntilStart(0);
        setTimeLeftSeconds(0);
        setWritingTimeLeftSeconds(0);
        setSubmissionTimeLeftSeconds(0);
        setIsWritingOver(true);
        setIsSubmissionWindowActive(false);
        setIsSubmissionWindowExpired(true);
        setExamEnded(true);
      }
    };
  ...
  }, [exam.StartTime, exam.EndTime, submissionWindowMinutes, submitSuccess]);
  ...
  const isTimeExpired = isSubmissionWindowExpired && !hasExtension;
  const isBlocked = !!activeDisallowOverride || isTerminatedByTeacher;
  // Students can submit during the examination writing time AND during the official submission grace period
  const canSubmit = !isBeforeStart && !isBlocked && (!isSubmissionWindowExpired || hasExtension);
  ...
  useEffect(() => {
    const unsubscribe = subscribeToStudentCommands(exam.ExamId, currentUser.UserId, (cmd: ProctorCommand) => {
      if (cmd.type === 'BLOCK') {
        ...
        const warningText = cmd.warningText || 'Please pay attention to the exam rules.';
        setActiveTeacherWarning(warningText);
        setActiveNotification({
          id: `proctor-warn-${Date.now()}`,
          title: 'Invigilator Notice & Directive',
          message: warningText,
          type: 'proctor',
          timestamp: new Date().toLocaleTimeString(),
          badge: 'FACULTY PROCTOR WARNING',
        });
  ...
  useEffect(() => {
    ...
    const handleWindowBlur = () => {
      logViolation('WINDOW_BLUR', 'Focus lost: active exam window blurred or application switched.');
    };
  ...
  const logViolation = (type: any, details: string) => {
    ...
    setActiveNotification({
      ...
      title: 'Surveillance Incident Logged',
      message: details,
      type: 'warning',
      timestamp: new Date().toLocaleTimeString(),
      badge: `${String(type).replace(/_/g, ' ')} DETECTED`,
    });
  ...
  // 8. Handle Student Answer Sheet file selection & validation
  const processSelectedFile = async (file: File) => {
    ...
    if (file.size > 30 * 1024 * 1024) {
      setErrorMsg('File size exceeds 30MB limit. Please compress your PDF.');
      setSelectedFile(null);
      setFileBase64(null);
      return;
    }
    ...
    setFileReadPercent(10);
    setFileReadLoaded(0);
    setFileReadStatus('Buffering & verifying PDF document...');

    try {
      const b64 = await fileToBase64WithProgress(file, (p) => {
        setFileReadPercent(p.percent);
        setFileReadLoaded(p.loaded);
        if (p.statusText) {
          setFileReadStatus(p.statusText);
        }
      });
      setFileBase64(b64);
      setIsFileReading(false);
      setFileReadStatus('PDF document verified and ready for submission!');
      // Asynchronously cache without blocking UI thread
      saveSubmissionFileStorage(exam.ExamId, currentUser.UserId, b64).catch(() => {});
    } catch (err) {
      setIsFileReading(false);
      setErrorMsg('Failed to process file locally. Please try selecting the file again.');
    }
  };
  ...
  const performFinalSubmission = async () => {
    ...
    setSubmitProgress({
      ...
      percent: 10,
      stage: 'preparing',
      statusText: 'Packaging and preparing answer booklet for secure transfer...',
    });

    try {
      const res = await executeGasAction(
        'submitAnswerSheet',
        {
          studentId: currentUser.UserId,
          studentName: currentUser.Name || currentUser.UserId,
      ...
      if (res.success) {
        ...
        setSubmitting(false);
        setErrorMsg(res.error || 'Failed to complete answer sheet submission. Please try again.');
      }
    } catch (err: any) {
      setSubmitting(false);
  ...
  const fetchDoubts = async () => {
    try {
      ...
      if (Array.isArray(doubts)) {
        ...
        // Detect if any doubt now has a new or updated reply from invigilator
        for (const currentDoubt of studentDoubts) {
          const replyText = currentDoubt.Answer?.trim() || '';
          if (currentDoubt.Status === 'ANSWERED' && replyText) {
            const replyKey = `${currentDoubt.DoubtId}__${replyText}`;
            if (!notifiedReplyKeysRef.current.has(replyKey)) {
              notifiedReplyKeysRef.current.add(replyKey);

              // If this was not the very first fetch on mount, trigger the pop-up and beep
              if (previousDoubtsRef.current !== null) {
                const snippet = replyText.length > 90 ? `${replyText.slice(0, 90)}...` : replyText;

                // Play distinct student reply alert beep sound
                playStudentReplyBeepSound();

                setActiveNotification({
                  id: `doubt_reply_${currentDoubt.DoubtId}_${Date.now()}`,
                  title: 'Invigilator Clarification Received',
                  message: `"${snippet}"`,
                  type: 'doubt_reply',
                  timestamp: new Date().toLocaleTimeString(),
                  badge: 'INVIGILATOR RESPONSE',
                  doubtQuestion: currentDoubt.Question,
                  doubtReplySnippet: snippet,
                  fullReply: replyText,
                });
                break; // Notify about the latest clarified inquiry
              }
            }
          }
        }

        previousDoubtsRef.current = studentDoubts;
  ...
  const [resolvedQpUrl, setResolvedQpUrl] = useState<string>(exam.QPUrl || '');

  useEffect(() => {
    let active = true;
    async function resolveQP() {
      if (exam.QPUrl && exam.QPUrl.trim() !== '') {
        if (exam.QPUrl.startsWith('data:')) {
          const blobUrl = base64ToBlobUrl(exam.QPUrl);
          if (active && blobUrl) {
            setResolvedQpUrl(blobUrl);
            return;
          }
        }
        if (active) setResolvedQpUrl(exam.QPUrl);
        return;
      }

      // Check IndexedDB
      try {
        const stored = await getQuestionPaperStorage(exam.ExamId);
        if (stored && active) {
          const clean = stored.startsWith('data:') ? stored : `data:application/pdf;base64,${stored}`;
          const blobUrl = base64ToBlobUrl(clean);
          if (blobUrl) {
            setResolvedQpUrl(blobUrl);
          } else {
            setResolvedQpUrl(clean);
          }
        }
      } catch (err) {
        console.warn('Error loading QP from storage:', err);
      }
    }
    resolveQP();
    return () => {
  ...
  }, [exam.ExamId, exam.QPUrl]);

  const effectiveQpUrl = resolvedQpUrl || exam.QPUrl || '';
  const qpEmbedInfo = getEmbeddablePdfUrl(effectiveQpUrl);
  ...
  return (
    <div className="min-h-screen bg-[#F4F8FA] flex flex-col font-sans select-none relative">
      ...
      {/* STUDENT EXAM ROOM NOTIFICATION POPUP MODAL */}
      {(activeNotification || activeTeacherWarning || lastWarning) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          <div className="relative w-full max-w-md bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden p-6 text-center space-y-4 font-sans animate-in zoom-in-95 duration-150">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              ...
              <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-700">
                {activeNotification?.timestamp || new Date().toLocaleTimeString()}
              </span>
            </div>
            ...
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mx-auto shadow-inner ${
              (activeNotification?.type === 'alert')
                ...
                : (activeNotification?.type === 'doubt_reply')
                ? 'bg-emerald-100 text-emerald-600 border border-emerald-200'
                : (activeNotification?.type === 'info')
                ? 'bg-sky-100 text-[#009fe3] border border-sky-200'
            ...
            <div className="space-y-1.5">
              <span className={`px-2.5 py-0.5 text-[10px] font-mono font-bold rounded-md uppercase tracking-wider inline-block ${
                (activeNotification?.type === 'alert')
                ...
                {activeNotification?.badge || (activeTeacherWarning ? 'INVIGILATOR DIRECTIVE' : 'PROCTORING SURVEILLANCE')}
              </span>
              <h3 className="text-base font-bold text-slate-900">
                {activeNotification?.title || (activeTeacherWarning ? 'Proctor Warning Issued' : 'Surveillance Incident Logged')}
              </h3>
            </div>
            ...
            <div className={`p-4 rounded-2xl text-xs text-left leading-relaxed border ${
              ...
              {activeNotification?.type === 'doubt_reply' ? (
                <div className="space-y-2">
                  {activeNotification.doubtQuestion && (
                    <div className="text-[11px] text-slate-500 pb-1.5 border-b border-emerald-200/60">
                      <span className="font-semibold text-slate-700">Your Inquiry:</span> {activeNotification.doubtQuestion}
                    </div>
                  )}
                  <div>
                    <span className="font-bold text-emerald-900 block mb-0.5 text-[11px] uppercase tracking-wide">
                      Official Invigilator Clarification:
                    </span>
                    <p className="font-semibold text-slate-900 text-xs italic bg-white/80 p-2.5 rounded-xl border border-emerald-200/60">
                      &ldquo;{activeNotification.doubtReplySnippet || activeNotification.message}&rdquo;
                    </p>
                  </div>
                  <p className="text-[11px] text-emerald-700 pt-1">
                    You can view the entire conversation history in the <strong>Ask Invigilator</strong> drawer.
                  </p>
                </div>
              ) : (
                <>
                  <p className="font-semibold mb-1">
                    {activeNotification?.message || activeTeacherWarning || lastWarning}
                  </p>
                  <p className="text-[11px] opacity-80 mt-2 pt-2 border-t border-current/10">
                    {activeTeacherWarning
                      ? 'This notice was transmitted directly by your invigilator. Repeated non-compliance will prompt remote test termination.'
                      : 'Browser tab switches, window minimizations, and clipboard actions are logged for invigilator review. Please keep this exam window focused and active.'}
                  </p>
                </>
              )}
            </div>
            ...
            <div className="flex gap-2">
              {activeNotification?.type === 'doubt_reply' && (
                <button
                  ...
                  className="flex-1 py-3 px-3 rounded-2xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 font-bold text-xs transition-all cursor-pointer min-h-[44px] flex items-center justify-center gap-1.5"
                >
              ...
              <button
                ...
                className={`py-3 px-4 rounded-2xl text-white font-bold text-xs shadow-md transition-all cursor-pointer min-h-[44px] flex items-center justify-center gap-2 ${
                  activeNotification?.type === 'doubt_reply'
                    ? 'flex-1 bg-emerald-600 hover:bg-emerald-700 shadow-emerald-600/20'
                    : 'w-full bg-[#009fe3] hover:bg-[#0088c4] shadow-sky-500/20'
                }`}
              >
                <span>{activeNotification?.type === 'doubt_reply' ? 'Got it' : 'I Acknowledge & Return to Examination'}</span>
      ...
      <div className="bg-[#0A192F] text-white px-4 sm:px-6 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sticky top-0 z-30 shadow-md border-b border-sky-900/50">
        ...
        <div className="flex items-center justify-between sm:justify-center gap-2.5 sm:gap-3">
          {/* Real-time Clock */}
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border shadow-xs transition-colors ${
            isBeforeStart
              ? 'bg-slate-900 border-amber-850'
              : isSubmissionWindowActive
              ? 'bg-amber-950/90 border-amber-500/80 shadow-amber-500/10'
              : isSubmissionWindowExpired
              ? 'bg-rose-950/80 border-rose-900/60'
              : timeLeftSeconds < 600
              ? 'bg-slate-900 border-orange-900/60'
            ...
            <Clock className={`w-4 h-4 ${
              isBeforeStart
                ...
                ? 'text-rose-400'
                : timeLeftSeconds < 600
                ? 'text-[#f25f22] animate-pulse'
                : 'text-[#009fe3]'
            }`} />
            <div>
              <div className={`font-mono font-bold text-xs sm:text-sm leading-none ${
                isBeforeStart
                  ? 'text-amber-400'
                  : isSubmissionWindowActive
                  ? 'text-amber-300'
              ...
              <div className={`text-[8px] uppercase tracking-wider font-semibold mt-0.5 ${
                isSubmissionWindowActive ? 'text-amber-400 font-bold' : 'text-slate-400'
              }`}>
                {isBeforeStart
                  ? 'Starts In'
                  : isSubmissionWindowActive
                  ? `Submission Window (${submissionWindowMinutes}m)`
                  : isSubmissionWindowExpired
                  ? 'Time Expired'
                  : 'Remaining'}
              </div>
          ...
          <div className="hidden lg:flex items-center">
            {isBeforeStart ? (
              <div className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl border bg-amber-950/80 text-amber-300 border-amber-800">
                <Lock className="w-3.5 h-3.5 text-amber-400" />
                <span>Starts Soon (Locked)</span>
              </div>
            ) : isBlocked ? (
              <div className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl border bg-rose-950/80 text-rose-300 border-rose-800">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl border bg-amber-500/20 text-amber-300 border-amber-500/60 hover:bg-amber-500/30 transition-colors animate-pulse cursor-pointer"
              >
                <UploadCloud className="w-3.5 h-3.5 text-amber-400" />
                <span>Submission Window Active</span>
              </button>
            ) : !canSubmit ? (
              <div className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl border bg-orange-950/80 text-[#f25f22] border-orange-800">
                <Lock className="w-3.5 h-3.5" />
                <span>Closed</span>
              </div>
            ) : hasExtension ? (
              <div className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl border bg-sky-950/80 text-sky-300 border-sky-800">
                <Unlock className="w-3.5 h-3.5 text-[#009fe3]" />
                <span>Extension Active</span>
              </div>
            ) : (
              <div className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl border bg-emerald-950/80 text-emerald-300 border-emerald-800">
      ...
      {/* Submission Window Active Alert Banner (High-Visibility Post-Exam Window) */}
      {isSubmissionWindowActive && (
        <div className="bg-gradient-to-r from-amber-600 via-orange-600 to-amber-700 text-white px-4 sm:px-6 py-2.5 shadow-md flex flex-wrap items-center justify-between gap-3 animate-in fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold text-xs sm:text-sm flex items-center gap-2 flex-wrap">
                <span>Exam Writing Concluded — Submission Window Active</span>
                <span className="bg-black/30 text-amber-200 px-2 py-0.5 rounded text-[10px] font-mono font-bold">
                  {formatTime(submissionTimeLeftSeconds)} Remaining
                </span>
                <span className="bg-white/20 text-white px-2 py-0.5 rounded text-[10px] uppercase font-bold">
                  Upload Only • Writing Prohibited
                </span>
              </div>
              <p className="text-[11px] text-amber-100 mt-0.5 leading-snug">
                Question paper access is aborted. Writing on answer sheets must cease immediately. Candidates have a {submissionWindowMinutes}-minute window (max 10m) solely to scan and upload their answer sheet.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setIsSubmissionModalOpen(true)}
              className="px-3.5 py-1.5 rounded-xl bg-white text-orange-950 hover:bg-amber-50 font-bold text-xs shadow-xs transition-colors flex items-center gap-1.5 cursor-pointer"
            >
              <UploadCloud className="w-3.5 h-3.5 text-[#f25f22]" />
      ...
      {isBlocked ? (
        ...
        <div className="bg-gradient-to-r from-orange-600 to-rose-600 text-white px-4 py-2.5 text-xs flex items-center justify-between shadow-xs font-medium">
          <div className="flex items-center gap-2">
            <Lock className="w-4 h-4 text-white shrink-0" />
            <span>
              <strong>Examination &amp; Submission Window Concluded:</strong> The {submissionWindowMinutes}-minute submission grace period has ended. Submissions are locked. If you encountered technical difficulties, request an extension permission from your instructor.
            </span>
          </div>
        </div>
      ) : hasExtension ? (
      ...
      <div className="lg:hidden px-4 pt-3 pb-1 bg-white border-b border-slate-200 grid grid-cols-2 gap-2">
        ...
        >
          {isWritingOver ? <FileX className="w-4 h-4 text-rose-300" /> : <FileText className="w-4 h-4" />}
          <span>{isWritingOver ? '1. QP (Aborted)' : '1. Question Paper'}</span>
        </button>
        <button
        ...
        >
          ...
          <span>{isSubmissionWindowActive ? '2. Submit Answer Sheet (Open)' : '2. Answer Sheet & Scan'}</span>
        </button>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-5 p-4 sm:p-6 max-w-7xl mx-auto w-full">
        ...
        >
          <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between gap-2 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-800 uppercase tracking-wider">
                {isBeforeStart
                  ? 'Question Paper (Encrypted Vault)'
                  : isWritingOver
                  ? 'Question Paper (Aborted & Revoked)'
            ...
            <div className="flex items-center gap-2">
              {isBeforeStart ? (
                ...
                <span className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-rose-800 bg-rose-50 rounded-xl border border-rose-200">
                  <FileX className="w-3.5 h-3.5 text-rose-600" />
                  <span>Access Revoked — Writing Over</span>
                </span>
              ) : (
                <>
                  <a
                    href={qpEmbedInfo.directDownloadUrl || effectiveQpUrl}
                  ...
                  <button
                    onClick={() => {
                      setPreviewPdfUrl(effectiveQpUrl);
          ...
          {/* Embedded Viewer Container OR High-Security Lock Vault OR Aborted Paper Screen */}
          <div className="flex-1 bg-slate-800 min-h-[480px] lg:min-h-[580px] flex flex-col relative">
            {isBeforeStart ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-6 sm:p-10 bg-slate-900 text-white space-y-5">
                ...
                <div className="max-w-md space-y-2">
                  <h3 className="text-lg font-bold text-white tracking-tight">
                    Writing Time Concluded — Question Paper Aborted
                  </h3>
                  <p className="text-xs text-slate-300 leading-relaxed">
                    Under examination regulations, question paper access has been terminated because the writing time has expired. Candidates are strictly prohibited from continuing to write.
                  </p>
                </div>

                {/* Submission Window Countdown Card */}
                {isSubmissionWindowActive ? (
                  <div className="p-4 rounded-2xl bg-slate-950 border border-amber-500/40 text-amber-400 max-w-sm w-full shadow-inner space-y-2">
                    <div className="text-[10px] uppercase font-bold tracking-widest text-slate-400 flex items-center justify-center gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-amber-400" />
                      <span>Submission Grace Window Active</span>
                    </div>
                    <div className="text-3xl sm:text-4xl font-mono font-black tracking-wider text-amber-300">
                      {formatTime(submissionTimeLeftSeconds)}
                    </div>
                    <div className="text-[11px] text-amber-200/90 font-mono">
                      Grace Duration: {submissionWindowMinutes} Minutes (Max 10)
                    </div>
                    <p className="text-[10px] text-slate-400 pt-1 leading-normal">
                      Scan and upload your answer booklet now. Do not write further on your sheets.
                    </p>
                    <div className="pt-2 flex flex-col sm:flex-row gap-2">
                      <button
                        type="button"
                        onClick={() => setIsSubmissionModalOpen(true)}
                        className="flex-1 py-2 px-3 rounded-xl bg-[#f25f22] hover:bg-[#ea580c] text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-xs transition-colors cursor-pointer"
                      >
                        <UploadCloud className="w-3.5 h-3.5" />
                        <span>Open Submission Popup</span>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        <FileImage className="w-3.5 h-3.5 text-[#009fe3]" />
                        <span>Scan Photos</span>
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="p-4 rounded-2xl bg-slate-950 border border-rose-500/40 text-rose-400 max-w-sm w-full shadow-inner space-y-1">
                    <div className="text-[10px] uppercase font-bold tracking-widest text-slate-400">
                      Submission Portal Status
                    </div>
                    <div className="text-xl font-mono font-bold text-rose-300">
                      Submission Window Elapsed
                    </div>
                    <p className="text-[11px] text-slate-400 mt-1">
                      The {submissionWindowMinutes}-minute submission grace period has ended. Submissions are locked.
                    </p>
                  </div>
                )}
              </div>
            ) : effectiveQpUrl ? (
              <div className="w-full h-full min-h-[520px] flex-1 flex flex-col bg-slate-950 overflow-hidden">
                <PdfCanvasViewer
                  url={effectiveQpUrl}
                  examId={exam.ExamId}
                  title={`Question Paper - ${exam.Subject || exam.ExamId}`}
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex-1 flex flex-col justify-between space-y-4">
            <div className="space-y-4">
              ...
              <label
                ...
                className={`border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center text-center cursor-pointer transition-all group ${
                  isDraggingFile
                    ? 'border-[#009fe3] bg-sky-50 ring-4 ring-sky-200 scale-[1.01]'
                    : highlightUploadBox
              ...
              >
                ...
                <UploadCloud className={`w-6 h-6 mb-1 transition-colors ${
                  isDraggingFile ? 'text-[#009fe3] animate-bounce' : highlightUploadBox ? 'text-orange-500' : 'text-slate-400 group-hover:text-[#009fe3]'
                }`} />
                <span className="text-xs font-semibold text-slate-800">
                  {isDraggingFile
                    ? 'Drop PDF Answer Booklet Here'
                    : selectedFile
                    ? selectedFile.name
                ...
                <span className="text-[10px] text-slate-500 mt-0.5">
                  {selectedFile
                    ...
                    : 'Supports standard PDF documents up to 30MB'}
                </span>
              </label>

              {/* PDF File Reading / Optimization Progress */}
              {isFileReading && (
                <div className="p-3.5 rounded-xl bg-sky-50/90 border border-sky-200 space-y-2 animate-in fade-in">
                  <div className="flex items-center justify-between text-xs font-bold text-sky-950">
                    <span className="flex items-center gap-1.5 truncate">
                      <Loader2 className="w-3.5 h-3.5 animate-spin text-[#009fe3] shrink-0" />
                      <span className="truncate">{fileReadStatus}</span>
                    </span>
                    <span className="font-mono text-[#009fe3] shrink-0 font-black ml-2">{fileReadPercent}%</span>
                  </div>
                  <div className="w-full h-2 bg-sky-200/60 rounded-full overflow-hidden p-0.5">
                    <div
                      className="h-full bg-gradient-to-r from-[#009fe3] to-sky-500 rounded-full transition-all duration-150"
                      style={{ width: `${Math.max(8, fileReadPercent)}%` }}
                    />
                  </div>
                  <div className="flex items-center justify-between text-[11px] text-slate-500">
                    <span>Reading &amp; validating document</span>
                    <span className="font-mono">
                      {selectedFile ? `${((fileReadLoaded || 0) / 1024 / 1024).toFixed(1)} / ${(selectedFile.size / 1024 / 1024).toFixed(1)} MB` : ''}
                    </span>
                  </div>
                </div>
              )}
                ...
                <div className="p-3.5 rounded-xl bg-emerald-50 border border-emerald-200 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 truncate pr-2">
                      ...
                      <div className="truncate">
                        <p className="text-xs font-bold text-emerald-950 truncate">{selectedFile.name}</p>
                        <p className="text-[10px] text-emerald-700 font-mono">
                          {(selectedFile.size / 1024 / 1024).toFixed(2)} MB • PDF Verified &amp; Ready
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 pt-1 border-t border-emerald-200/60">
                    <button
                      ...
                      className="flex-1 py-1.5 px-2.5 bg-white hover:bg-emerald-100/60 text-emerald-800 border border-emerald-300 rounded-lg text-xs font-semibold flex items-center justify-center gap-1 transition-colors cursor-pointer"
                    >
                      <Eye className="w-3.5 h-3.5 text-emerald-600" />
                      <span>Review PDF</span>
                    </button>
                    <label className="py-1.5 px-2.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-300 rounded-lg text-xs font-semibold flex items-center justify-center gap-1 transition-colors cursor-pointer shrink-0">
                      <input
                        type="file"
                        accept="application/pdf"
                        onChange={handleFileChange}
              ...
              {/* Live Upload Progress Indicator in Sidebar */}
              {submitting && (
                <div className="p-3.5 rounded-xl bg-gradient-to-br from-sky-50 to-blue-50/70 border-2 border-[#009fe3]/40 shadow-sm space-y-2.5 animate-in fade-in">
                  <div className="flex items-center justify-between text-xs font-bold text-slate-900">
                    <span className="flex items-center gap-1.5 truncate">
                      <Loader2 className="w-4 h-4 animate-spin text-[#009fe3] shrink-0" />
                      <span className="truncate">{submitProgress.statusText || 'Submitting answer booklet...'}</span>
                    </span>
                    <span className="font-mono text-[#009fe3] shrink-0 font-black text-sm ml-2">
                      {submitProgress.percent}%
                    </span>
                  </div>
                  <div className="w-full h-3 bg-sky-200/70 rounded-full overflow-hidden p-0.5 shadow-inner">
                    <div
                      className="h-full bg-gradient-to-r from-[#009fe3] via-sky-500 to-emerald-500 rounded-full transition-all duration-300 relative overflow-hidden"
                      style={{ width: `${Math.max(6, submitProgress.percent)}%` }}
                    >
                      <div className="absolute inset-0 bg-white/25 animate-[shimmer_1.2s_infinite] -skew-x-12" />
                    </div>
                  </div>
                  <div className="flex items-center justify-between text-[11px] text-slate-600">
                    ...
                    <span className="font-mono font-semibold text-slate-700">
                      {submitProgress.total > 100
                        ? `${(submitProgress.loaded / (1024 * 1024)).toFixed(2)} / ${(submitProgress.total / (1024 * 1024)).toFixed(2)} MB`
                        : `${submitProgress.percent}%`}
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-1 pt-1 border-t border-sky-200/60 text-[10px] text-center">
                    <div className={`p-1 rounded font-medium ${submitProgress.percent >= 15 ? 'bg-sky-100 text-sky-900 font-bold' : 'text-slate-400'}`}>
                      1. Encrypt
                    </div>
                    <div className={`p-1 rounded font-medium ${submitProgress.percent >= 25 && submitProgress.percent < 88 ? 'bg-[#009fe3] text-white font-bold animate-pulse' : submitProgress.percent >= 88 ? 'bg-sky-100 text-sky-900 font-bold' : 'text-slate-400'}`}>
                      2. Upload
                    </div>
                    <div className={`p-1 rounded font-medium ${submitProgress.percent >= 88 ? 'bg-emerald-100 text-emerald-900 font-bold animate-pulse' : 'text-slate-400'}`}>
                      3. Register
                    </div>
                  </div>
                </div>
      ...
      {/* Submission Window Popup Modal (Auto-triggered post-exam) */}
      <SubmissionWindowModal
        isOpen={isSubmissionModalOpen}
        onClose={() => setIsSubmissionModalOpen(false)}
        submissionTimeLeftSeconds={submissionTimeLeftSeconds}
        submissionWindowMinutes={submissionWindowMinutes}
        isSubmissionWindowExpired={isSubmissionWindowExpired}
        hasExtension={hasExtension}
        canSubmit={canSubmit}
        selectedFile={selectedFile}
        fileBase64={fileBase64}
        isFileReading={isFileReading}
        fileReadPercent={fileReadPercent}
        ...
        errorMsg={errorMsg}
        isMobile={isMobile}
        onOpenScanner={() => setImageToPdfOpen(true)}
        onFileChange={handleFileChange}
        onReviewPdf={() => {
          if (fileBase64) {
            setPreviewPdfUrl(fileBase64);
            setPreviewPdfTitle(`Uploaded Answer Sheet - ${currentUser.Name} (${currentUser.UserId})`);
          }
        }}
        onSubmitFinal={() => {
          if (!selectedFile) {
            setErrorMsg('Please attach or scan your answer sheet PDF before submitting.');
            return;
          }
          setIsConfirmSubmitOpen(true);
        }}
      ...
      {/* Persistent Floating Bottom Bar for Active Submission Grace Window */}
      {isSubmissionWindowActive && !isSubmissionModalOpen && !submitSuccess && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 w-full max-w-lg px-4 pointer-events-none animate-in slide-in-from-bottom duration-300">
          <div className="bg-slate-950/95 text-white border-2 border-amber-500/80 rounded-2xl p-3 shadow-2xl backdrop-blur-md flex items-center justify-between gap-3 pointer-events-auto">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <div className="text-xs font-bold text-amber-400 flex items-center gap-1.5 truncate">
                  <span>Submission Window Active</span>
                  <span className="font-mono text-white bg-amber-500/30 px-1.5 py-0.5 rounded text-[10px]">
                    {formatTime(submissionTimeLeftSeconds)} Left
                  </span>
                </div>
                <p className="text-[10px] text-slate-300 truncate">
                  Writing stopped. Submit answer sheet now ({submissionWindowMinutes}m grace).
                </p>
              </div>
            </div>
            <button
              type="button"
              onClick={() => setIsSubmissionModalOpen(true)}
              className="px-3.5 py-2 rounded-xl bg-[#f25f22] hover:bg-[#ea580c] text-white font-bold text-xs flex items-center gap-1.5 shrink-0 transition-colors shadow-sm cursor-pointer"
            >
              <UploadCloud className="w-3.5 h-3.5" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-200 font-sans">
            ...
            <div className="bg-[#0A192F] p-4 text-white flex items-center justify-between border-b border-sky-900">
              <div className="flex items-center gap-2">
                <div className={`p-1.5 rounded-lg ${submitting ? 'bg-sky-500/20 text-[#009fe3]' : submitSuccess ? 'bg-emerald-500/20 text-emerald-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
                  {submitting ? (
            ...
            <div className="p-5 space-y-4 text-xs">
              {submitting || submitSuccess ? (
                /* Active Upload & Submission Progress State */
                <div className="py-4 flex flex-col items-center text-center space-y-4 animate-in fade-in">
                  <div className="relative flex items-center justify-center">
                    <div className="w-16 h-16 rounded-full bg-sky-50 border-2 border-sky-100 flex items-center justify-center">
                      {submitSuccess ? (
                        ...
                        <UploadCloud className="w-8 h-8 text-[#009fe3] animate-pulse" />
                      )}
                    </div>
                    {!submitSuccess && (
                      <div className="absolute inset-0 rounded-full border-2 border-dashed border-[#009fe3] animate-spin" style={{ animationDuration: '4s' }}></div>
                    )}
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-center gap-2 mb-1">
                      <span className="text-3xl font-black text-slate-900 font-mono tracking-tight">
                        {submitSuccess ? 100 : submitProgress.percent}%
                      </span>
                    </div>
                    <p className="text-xs font-semibold text-slate-700">
                      {submitSuccess
                        ? 'Answer booklet securely logged in institutional register!'
                        : submitProgress.statusText || 'Transferring answer booklet to cloud vault...'}
                    </p>
                    <p className="text-[11px] text-slate-500 mt-1">
                      {submitSuccess
                        ? 'Concluding proctored examination session...'
                        : 'Please do not navigate away or refresh this window.'}
                    </p>
                  </div>
                  ...
                  <div className="w-full space-y-1.5">
                    <div className="w-full h-3.5 bg-slate-100 rounded-full overflow-hidden p-0.5 border border-slate-200 shadow-inner">
                      <div
                        className="h-full rounded-full bg-gradient-to-r from-[#009fe3] via-sky-500 to-emerald-500 transition-all duration-300 relative overflow-hidden"
                        style={{ width: `${Math.max(8, submitSuccess ? 100 : submitProgress.percent)}%` }}
                      >
                        {!submitSuccess && (
                    ...
                    <div className="flex justify-between text-[11px] text-slate-500 font-mono px-0.5">
                      ...
                      <span>
                        {submitProgress.total > 100
                          ? `${(submitProgress.loaded / 1024 / 1024).toFixed(1)} / ${(submitProgress.total / 1024 / 1024).toFixed(1)} MB`
                          : `${submitProgress.percent}%`}
                      </span>
                    </div>
                  </div>
                  ...
                  {/* Milestone Stages */}
                  <div className="w-full bg-slate-50 rounded-xl p-3 border border-slate-200 text-left space-y-2">
                    <div className="flex items-center gap-2">
                      <Check className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                      <span className="text-[11px] font-medium text-slate-800">1. Answer booklet verified &amp; sealed</span>
                    </div>
                    <div className="flex items-center gap-2">
                      {submitProgress.percent >= 85 || submitSuccess ? (
                        <Check className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                      ) : (
                        <Loader2 className="w-3.5 h-3.5 text-[#009fe3] animate-spin shrink-0" />
                      )}
                      <span className={`text-[11px] font-medium ${submitProgress.percent >= 85 || submitSuccess ? 'text-slate-800' : 'text-[#009fe3] font-bold'}`}>
                        2. High-speed upload to institutional Drive
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      {submitSuccess ? (
                        <Check className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                      ) : submitProgress.percent >= 85 ? (
                        <Loader2 className="w-3.5 h-3.5 text-amber-500 animate-spin shrink-0" />
                      ) : (
                        <div className="w-3.5 h-3.5 rounded-full border border-slate-300 shrink-0"></div>
                      )}
                      <span className={`text-[11px] font-medium ${submitSuccess ? 'text-slate-800' : submitProgress.percent >= 85 ? 'text-amber-700 font-bold' : 'text-slate-400'}`}>
                        3. Archive in Examination Register &amp; sign out
                      </span>
                    </div>
                  </div>
                </div>
