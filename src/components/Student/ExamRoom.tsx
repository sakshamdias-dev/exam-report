...
export const ExamRoom: React.FC<ExamRoomProps> = ({ exam, currentUser, onExit, onSubmitted }) => {
  ...
  const [activeNotification, setActiveNotification] = useState<{
    ...
    title: string;
    message: string;
    type: 'warning' | 'alert' | 'info' | 'proctor';
    timestamp: string;
    badge?: string;
  } | null>(null);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          <div className="relative w-full max-w-md bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden p-6 text-center space-y-4 font-sans animate-in zoom-in-95 duration-150">
            ...
            <button
              ...
              className="w-full py-3 px-4 rounded-2xl bg-[#009fe3] hover:bg-[#0088c4] text-white font-bold text-xs shadow-md shadow-sky-500/20 transition-all cursor-pointer min-h-[44px] flex items-center justify-center gap-2"
            >
              <span>I Acknowledge &amp; Return to Examination</span>
