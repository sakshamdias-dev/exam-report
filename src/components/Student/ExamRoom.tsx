...
import { playDoubtAlertSound } from '../../utils/audioAlert';
import { 
  isMobileDevice, 
  ...
  const fetchDoubts = async () => {
    try {
      ...
      if (Array.isArray(doubts)) {
        ...
        // Detect if any previously open/unanswered doubt now has a reply
        if (previousDoubtsRef.current) {
          for (const currentDoubt of studentDoubts) {
            const prev = previousDoubtsRef.current.find((p) => p.DoubtId === currentDoubt.DoubtId);
            const isNewlyAnswered =
              currentDoubt.Status === 'ANSWERED' &&
              Boolean(currentDoubt.Answer?.trim()) &&
              (!prev || prev.Status !== 'ANSWERED' || !prev.Answer?.trim());

            if (isNewlyAnswered) {
              const replyText = currentDoubt.Answer?.trim() || '';
              const snippet = replyText.length > 90 ? `${replyText.slice(0, 90)}...` : replyText;

              // Play soft chime to alert student of the reply
              playDoubtAlertSound();
