...
interface ImageToPdfModalProps {
  isOpen: boolean;
  onClose: () => void;
  onPdfGenerated: (pdfFile: File, pdfBase64: string, rawImages?: string[]) => void;
  examSubject?: string;
  examId?: string;
  candidateId?: string;
...
export const ImageToPdfModal: React.FC<ImageToPdfModalProps> = ({
  ...
  const [compileProgress, setCompileProgress] = useState<{ percent: number; stage: string }>({
    percent: 0,
    stage: '',
  });
  ...
  const handleCompilePdf = async () => {
    ...
    try {
      ...
      setCompileProgress({ percent: 10, stage: 'Preparing answer sheets...' });
      ...
      const result = await generatePdfFromImages(
        pages,
        {
          ...
          maxDimension: pages.length > 5 ? 1150 : 1280,
          quality: pages.length > 5 ? 0.70 : 0.74,
        },
        (prog) => {
          setCompileProgress({
  ...
  const handleConfirmAndAttach = () => {
    ...
    onPdfGenerated(pdfFile, generatedPdf.base64, generatedPdf.pageImages);
    onClose();
  };
    ...
    >
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-2 sm:p-4 animate-in fade-in duration-150">
        <div className="relative w-full max-w-4xl max-h-[92vh] bg-white border border-slate-200 rounded-2xl flex flex-col shadow-2xl overflow-hidden font-sans">
          ...
          {isGenerating && (
            <div className="px-5 py-3 bg-indigo-50/80 border-t border-indigo-100 flex flex-col gap-1.5 animate-in fade-in">
              <div className="flex items-center justify-between text-xs font-semibold text-indigo-950">
                <span className="flex items-center gap-1.5 truncate">
                  ...
                  <span>{compileProgress.stage || 'Assembling high-speed PDF...'}</span>
                </span>
                <span className="font-mono text-indigo-600 font-bold ml-2 shrink-0">
              ...
              <div className="w-full h-2 bg-indigo-200/60 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.max(5, compileProgress.percent)}%` }}
                />
              </div>
            </div>
          )}
