...
import { Calendar, Trash2, Edit3, Eye, Award, Clock, AlertTriangle, Loader2, Users, Layers, UserCheck, ShieldCheck, KeyRound } from 'lucide-react';
import { PdfViewerModal } from '../PdfViewerModal';
import { SubmissionPermissionsModal } from './SubmissionPermissionsModal';
import { getQuestionPaperStorageSync } from '../../utils/fileStorage';
...
export const ExamsTable: React.FC<ExamsTableProps> = ({ exams, onRefresh }) => {
  ...
  const [permissionsExam, setPermissionsExam] = useState<Exam | null>(null);
  const [editStartTime, setEditStartTime] = useState('');
  const [editEndTime, setEditEndTime] = useState('');
  const [editSubmissionWindow, setEditSubmissionWindow] = useState<number>(10);
  ...
  const handleOpenEdit = async (exam: Exam) => {
    ...
    const savedWindow = exam.SubmissionWindowMinutes ?? (() => {
      try {
        const s = localStorage.getItem('exam_sub_window_' + exam.ExamId);
        return s ? Number(s) : 10;
      } catch {
        return 10;
      }
    })();
    setEditSubmissionWindow(Math.min(10, Math.max(1, savedWindow || 10)));
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    setSavingEdit(true);
    const clampedWindow = Math.min(10, Math.max(1, Number(editSubmissionWindow) || 10));
    try {
      localStorage.setItem('exam_sub_window_' + editingExam.ExamId, String(clampedWindow));
    } catch (e) {}
    ...
    try {
      const res = await executeGasAction('updateExam', {
        ...
        EndTime: new Date(editEndTime).toISOString(),
        submissionWindowMinutes: clampedWindow,
        SubmissionWindowMinutes: clampedWindow,
  ...
  return (
    <div id="exams-table-container" className="space-y-4 font-sans">
                  ...
                  return (
                    <tr key={exam.ExamId} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-slate-600 font-mono text-[11px]">
                        ...
                        <div className="text-sky-700 font-semibold text-[10px] mt-0.5">
                          Submission Grace: +{exam.SubmissionWindowMinutes ?? (() => {
                            try {
                              const s = localStorage.getItem('exam_sub_window_' + exam.ExamId);
                              return s ? Number(s) : 10;
                            } catch {
                      ...
                      <td className="px-4 py-3.5">
                        {exam.QPUrl || getQuestionPaperStorageSync(exam.ExamId) ? (
                          <button
                            onClick={() => {
                              setPreviewPdfUrl(exam.QPUrl || null);
                      ...
                      <td className="px-4 py-3.5 text-right">
                        <div className="inline-flex items-center gap-1.5">
                          <button
                            onClick={() => setPermissionsExam(exam)}
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-sky-50 text-sky-800 hover:bg-sky-100 border border-sky-300 font-semibold text-[11px] transition-colors shadow-2xs"
                            title="Manage Submission Permissions & Overrides for this Exam"
                          >
                          ...
                          <button
                            ...
                            className="p-1.5 rounded text-slate-500 hover:text-sky-700 hover:bg-slate-100 transition-colors"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-lg bg-white border border-slate-200 rounded-lg p-5 shadow-2xl space-y-4 font-sans max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveEdit} className="space-y-3.5 text-xs">
              ...
              {/* Submission Window Duration (Grace Period) */}
              <div className="p-3 bg-sky-50/70 border border-sky-200 rounded-lg space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-slate-800 font-semibold flex items-center gap-1.5 text-xs">
                    ...
                    <span>Answer Sheet Submission Window (Grace Period)</span>
                  </label>
                  <span className="font-mono font-bold text-xs text-[#009fe3]">
                    {editSubmissionWindow}m (Max 10m)
                  </span>
                </div>
                <p className="text-[11px] text-slate-600">
                  When End Time passes, the question paper is immediately aborted. Candidates get this duration strictly to upload their answer sheets without writing further.
                </p>
                <div className="flex items-center gap-2 pt-1">
                  <input
                    ...
                    min="1"
                    max="10"
                    value={editSubmissionWindow}
                    onChange={(e) => {
                      const val = parseInt(e.target.value, 10);
                      if (isNaN(val)) setEditSubmissionWindow(1);
                      else setEditSubmissionWindow(Math.min(10, Math.max(1, val)));
                    }}
                    className="w-24 px-3 py-1.5 bg-white border border-slate-300 rounded text-xs text-slate-900 font-mono focus:outline-hidden focus:border-blue-600"
                  />
                  <div className="flex items-center gap-1">
                    {[3, 5, 10].map((mins) => (
                      <button
                        key={mins}
                        type="button"
                        onClick={() => setEditSubmissionWindow(mins)}
                        className={`px-2.5 py-1 rounded text-xs font-semibold border transition-colors ${
                          editSubmissionWindow === mins
      ...
      {/* Submission Permissions & Overrides Modal */}
      {permissionsExam && (
        <SubmissionPermissionsModal
          isOpen={!!permissionsExam}
          onClose={() => setPermissionsExam(null)}
          exam={permissionsExam}
