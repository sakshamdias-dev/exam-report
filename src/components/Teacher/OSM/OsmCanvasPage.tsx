...
export const OsmCanvasPage: React.FC<OsmCanvasPageProps> = ({
  ...
  useEffect(() => {
    ...
    if (imageSrc) {
      ...
      const img = new Image();
      // CRITICAL: Only set crossOrigin on remote HTTP/HTTPS URLs (never on data: or blob: URIs)
      ...
      img.onerror = () => {
        if (isCancelled) return;
        // If it failed with crossOrigin on an HTTP URL, retry once without crossOrigin
        if (img.crossOrigin) {
