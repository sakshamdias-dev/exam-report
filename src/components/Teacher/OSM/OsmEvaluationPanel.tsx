...
interface OsmEvaluationPanelProps {
  ...
  onSaveDraft: () => void;
  onDownloadQuestionWiseReport?: () => void;
}
...
export const OsmEvaluationPanel: React.FC<OsmEvaluationPanelProps> = ({
  ...
  onFinishChecking,
  onSaveDraft,
  onDownloadQuestionWiseReport,
}) => {
  const [activeTab, setActiveTab] = useState<'marks' | 'feedback'>('marks');

  // Calculate question-level total evaluable marks and received marks
  const totalQuestionMarks = questionScores.reduce((sum, q) => sum + (Number(q.maxMarks) || 0), 0) || maxExamMarks;
  ...
  const isOverMax = totalAwardedMarks > totalQuestionMarks;
  const percentage = totalQuestionMarks > 0 ? Math.round((totalAwardedMarks / totalQuestionMarks) * 100) : 0;
    ...
    >
      ...
      {!isOpen ? (
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden text-xs">
          {/* Top Score Summary Banner: Total Question marks, Received marks & Grand total */}
          <div className="p-3 bg-slate-950 border-b border-slate-800 space-y-2">
            <div className="grid grid-cols-3 gap-1.5 text-center">
              <div className="p-1.5 rounded bg-slate-900 border border-slate-800">
                <span className="text-[9px] text-slate-400 block font-semibold leading-tight">Total Question Marks</span>
                <span className="font-mono text-sm font-bold text-sky-400 mt-0.5 block">{totalQuestionMarks}</span>
              </div>
              <div className="p-1.5 rounded bg-slate-900 border border-slate-800">
                <span className="text-[9px] text-slate-400 block font-semibold leading-tight">Received Marks</span>
                <span className={`font-mono text-sm font-bold mt-0.5 block ${isOverMax ? 'text-rose-400' : 'text-emerald-400'}`}>
                  {totalAwardedMarks}
                </span>
              </div>
              <div className="p-1.5 rounded bg-slate-900 border border-slate-800">
                <span className="text-[9px] text-slate-400 block font-semibold leading-tight">Grand Total</span>
                <span className="font-mono text-xs font-bold text-purple-300 mt-0.5 block truncate">
                  {totalAwardedMarks}/{totalQuestionMarks}
                </span>
                <span className="text-[8px] text-slate-400 block">{percentage}%</span>
              </div>
            </div>

            {/* Warning if exceeded max marks */}
            {isOverMax && (
              <div className="p-1.5 rounded bg-rose-950/80 border border-rose-800 text-rose-300 text-[10px] flex items-center gap-1.5">
                <AlertCircle className="w-3.5 h-3.5 shrink-0 text-rose-400" />
                <span>Received marks ({totalAwardedMarks}) exceed total question marks ({totalQuestionMarks})!</span>
              </div>
            )}
          </div>
          ...
          <div className="p-3 border-t border-slate-800 bg-slate-950 space-y-2">
            ...
            {onDownloadQuestionWiseReport && (
              <button
                type="button"
                onClick={onDownloadQuestionWiseReport}
                className="w-full py-1.5 px-3 rounded-lg bg-sky-950 hover:bg-sky-900 text-sky-300 border border-sky-800 font-semibold text-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                title="Generate and download Question-Wise Evaluation Report PDF with ef_logo.png"
              >
