...
import { generateCheckedPdfBlob, generateFallbackAnswerBooklet, generateStudentBookletPdf, generateQuestionWiseMarkingPdfBlob } from '../../../utils/osmPdfGenerator';
import { getEmbeddablePdfUrl, fetchDrivePdfBinary, isPdfOrImageBytes, uint8ArrayToBase64 } from '../../../utils/pdfHelper';
import { getSubmissionFileStorage, saveSubmissionFileStorage, saveGradedPaperStorage } from '../../../utils/fileStorage';
import { Submission, Exam } from '../../../types';
import { fileToBase64, executeGasAction } from '../../../services/api';
import { supabaseGetSubmissions } from '../../../services/supabaseDb';
import {
  RefreshCw,
  AlertTriangle,
  CheckCircle2,
...
// Setup pdfjs worker using local bundled Vite worker asset
try {
...
export const OsmWindow: React.FC<OsmWindowProps> = ({
  ...
  const [loadedPdfBytes, setLoadedPdfBytes] = useState<Uint8Array | null>(null);
  ...
  const [questionScores, setQuestionScores] = useState<OsmQuestionScore[]>(() => {
    let qmList = exam.QuestionMarks;
    if (!qmList || qmList.length === 0) {
      try {
        const stored = localStorage.getItem(`exam_qm_${exam.ExamId.toUpperCase()}`) || localStorage.getItem(`exam_qm_${exam.ExamId}`);
        if (stored) {
          const parsed = JSON.parse(stored);
          if (Array.isArray(parsed) && parsed.length > 0) qmList = parsed;
        }
      } catch (e) {}
    }

    if (qmList && qmList.length > 0) {
      return qmList.map((qm, idx) => ({
        id: qm.id || `q_${idx + 1}`,
  ...
  useEffect(() => {
    try {
      ...
      } else if (submission.OsmDraftData) {
        // Restore from Supabase submission record
        try {
          const parsed: OsmSessionData = typeof submission.OsmDraftData === 'string'
            ? JSON.parse(submission.OsmDraftData)
            : submission.OsmDraftData;
          if (parsed && typeof parsed === 'object') {
            ...
            setLastSavedText('Restored ink markings & scores from database record.');
          }
        } catch (dbDraftErr) {
          console.warn('Could not parse osm_draft_data from submission:', dbDraftErr);
        }
      } else {
        // Look up question bifurcation from exam or local cache
        let qmList = exam.QuestionMarks;
        if (!qmList || qmList.length === 0) {
          try {
            const stored = localStorage.getItem(`exam_qm_${exam.ExamId.toUpperCase()}`) || localStorage.getItem(`exam_qm_${exam.ExamId}`);
  ...
  }, [storageDraftKey, submission.Score, maxExamMarks, exam.ExamId, exam.QuestionMarks]);
  ...
  useEffect(() => {
    ...
    async function loadDocument() {
      try {
        ...
        // Check 1.8: If submission URL starts with stored://, fetch from storage
        if (submission.SubmissionUrl && submission.SubmissionUrl.startsWith('stored://')) {
          const stored = await getSubmissionFileStorage(exam.ExamId, submission.StudentId);
          if (stored?.rawImages && stored.rawImages.length > 0) {
            if (!isCancelled) {
              setImagePages(stored.rawImages);
              setTotalPages(stored.rawImages.length);
              setIsUsingDigitalBookletFallback(false);
              setFileSourceNotice(`Loaded ${stored.rawImages.length} student answer sheet page photo${stored.rawImages.length > 1 ? 's' : ''} from storage.`);
              setLoadingPdf(false);
            }
            return;
          }
          if (stored?.pdfBase64) {
            const base64 = stored.pdfBase64.split(',')[1] || stored.pdfBase64;
            const cleanB64 = base64.replace(/\s/g, '');
            const binaryStr = atob(cleanB64);
            const len = binaryStr.length;
            const bytes = new Uint8Array(len);
        ...
        // Case A: Data URL or raw Base64 PDF
        if (pdfUrl.startsWith('data:application/pdf;base64,') || pdfUrl.startsWith('data:') || pdfUrl.startsWith('JVBERi0')) {
          const base64 = pdfUrl.includes('base64,')
        ...
        // Case B: Direct URL - Attempt fetching with PDF.js (only for standard non-Drive URLs)
        const isDriveLink = pdfInfo.isDriveUrl || Boolean(rawUrl && (rawUrl.includes('drive.google.com') || rawUrl.includes('docs.google.com')));
        if (pdfUrl && (pdfUrl.startsWith('http://') || pdfUrl.startsWith('https://')) && !isDriveLink) {
          try {
            ...
            console.warn('Direct PDF load via URL encountered CORS/Fetch issue, falling back to gateways/proxies:', directFetchErr);
          }
        }

        // Case B.2: If Google Drive link, attempt to fetch binary via backend Apps Script gateway
        if (isDriveLink || pdfInfo.isDriveUrl || pdfInfo.driveFileId) {
          const driveId = pdfInfo.driveFileId || (rawUrl.match(/(?:file\/d\/|id=|open\?id=)([-\w]{15,})/)?.[1] || '');

          // Strategy 1: Apps Script backend gateway
          try {
            ...
            const b64Data = (gasRes as any).data?.base64Data || (gasRes as any).base64Data || (gasRes as any).data?.data || (gasRes as any).pdfBase64;
            if (gasRes.success && b64Data) {
              const cleanB64 = (b64Data.indexOf(',') > -1 ? b64Data.split(',')[1] : b64Data).replace(/\s/g, '');
              const binaryStr = atob(cleanB64);
              const len = binaryStr.length;
              const bytes = new Uint8Array(len);
              for (let i = 0; i < len; i++) {
              ...
              if (check.isPdf) {
                ...
                if (!isCancelled) {
                  ...
                  setTotalPages(doc.numPages || 1);
                  setIsUsingDigitalBookletFallback(false);
                  setFileSourceNotice('Loaded original student PDF via secure Apps Script gateway.');
                  setLoadingPdf(false);
                  return;
                }
              } else if (check.isImage) {
                const dataUrl = `data:${check.mimeType};base64,${cleanB64}`;
                if (!isCancelled) {
                  setImagePages([dataUrl]);
                  setTotalPages(1);
                  setIsUsingDigitalBookletFallback(false);
                  setFileSourceNotice('Loaded original student answer booklet photo via Apps Script gateway.');
                  setLoadingPdf(false);
                  try {
                    localStorage.setItem(`osm_student_images_${exam.ExamId}_${submission.StudentId}`, JSON.stringify([dataUrl]));
                  } catch (_) {}
                  saveSubmissionFileStorage(exam.ExamId, submission.StudentId, undefined, [dataUrl]);
                  return;
                }
              }
            }
          ...
          // Strategy 2: Multi-strategy direct/proxy fetch (bypass Google Drive CORS)
          if (driveId || rawUrl) {
            try {
              const driveBinary = await fetchDrivePdfBinary(driveId, rawUrl);
              if (driveBinary && driveBinary.bytes) {
                if (driveBinary.isPdf) {
                  const loadingTask = pdfjsLib.getDocument({ data: driveBinary.bytes });
                  const doc = await loadingTask.promise;
                  if (!isCancelled) {
                    setPdfDoc(doc);
                    setLoadedPdfBytes(driveBinary.bytes);
                    setTotalPages(doc.numPages || 1);
                    setIsUsingDigitalBookletFallback(false);
                    setFileSourceNotice('Loaded original student PDF via high-speed Drive gateway.');
                    setLoadingPdf(false);
                    return;
                  }
                } else if (driveBinary.isImage) {
                  const b64 = uint8ArrayToBase64(driveBinary.bytes);
                  const dataUrl = `data:${driveBinary.mimeType};base64,${b64}`;
                  if (!isCancelled) {
                    setImagePages([dataUrl]);
                    setTotalPages(1);
                    setIsUsingDigitalBookletFallback(false);
                    setFileSourceNotice('Loaded original student answer booklet photo via Drive gateway.');
                    setLoadingPdf(false);
                    try {
                      localStorage.setItem(`osm_student_images_${exam.ExamId}_${submission.StudentId}`, JSON.stringify([dataUrl]));
                    } catch (_) {}
              ...
              console.warn('fetchDrivePdfBinary error in OsmWindow:', proxyErr);
            }
          }
        }

        // Check local storage for student images or PDF before fallback
        try {
          const stored = await getSubmissionFileStorage(exam.ExamId, submission.StudentId);
          if (stored?.rawImages && stored.rawImages.length > 0) {
            if (!isCancelled) {
              ...
              setTotalPages(stored.rawImages.length);
              setIsUsingDigitalBookletFallback(false);
              setFileSourceNotice(`Loaded ${stored.rawImages.length} student answer sheet page photo${stored.rawImages.length > 1 ? 's' : ''} from local storage.`);
              setLoadingPdf(false);
              return;
            }
          }
          if (stored?.pdfBase64) {
            const cleanB64 = (stored.pdfBase64.indexOf(',') > -1 ? stored.pdfBase64.split(',')[1] : stored.pdfBase64).replace(/\s/g, '');
            const binaryStr = atob(cleanB64);
            const len = binaryStr.length;
            const bytes = new Uint8Array(len);
            for (let i = 0; i < len; i++) {
            ...
            if (!isCancelled) {
              ...
              setTotalPages(doc.numPages || 1);
              setIsUsingDigitalBookletFallback(false);
              setFileSourceNotice('Loaded student answer booklet PDF directly from local storage.');
              setLoadingPdf(false);
              return;
            }
          }
        ...
        if (!isCancelled) {
          setPdfDoc(doc);
          setLoadedPdfBytes(fallbackBytes);
        ...
        if (!isCancelled) {
          ...
          try {
            ...
            setPdfDoc(doc);
            setLoadedPdfBytes(emergencyBytes);
  ...
  }, [submission.SubmissionUrl, customPdfBase64, imagePages, submission.StudentId, submission.StudentName, exam.ExamId, exam.Subject, loadAttempt]);
  ...
  const handleUploadLocalPdf = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await saveSubmissionFileStorage(exam.ExamId, submission.StudentId, b64, undefined);
    } catch (err) {
  ...
  const handleUploadLocalImages = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await saveSubmissionFileStorage(exam.ExamId, submission.StudentId, undefined, base64List);
    } catch (err: any) {
  ...
  const handleFinishChecking = async () => {
    try {
      if (isUsingDigitalBookletFallback) {
        const proceed = window.confirm(
          "Notice: The student's original uploaded answer booklet was not retrieved from Google Drive (currently showing a blank template).\n\nIf you finish now, your markings will be saved onto this blank template.\n\nAre you sure you want to proceed and save?"
        );
        if (!proceed) {
          return;
        }
      ...
      // Determine source document payload with strict priority to student's genuine submission
      let sourceDocPayload: any = null;
      if (!isUsingDigitalBookletFallback && loadedPdfBytes) {
        sourceDocPayload = loadedPdfBytes;
      } else if (!isUsingDigitalBookletFallback && pdfDoc && typeof pdfDoc.getData === 'function') {
        try {
          sourceDocPayload = await pdfDoc.getData();
        } catch (pdfDocErr) {
          console.warn('Could not extract bytes directly from pdfDoc proxy:', pdfDocErr);
        }
      }

      if (!sourceDocPayload) {
        sourceDocPayload = customPdfBase64 || submission.SubmissionUrl || '';
      }

      // If still missing or previously flagged as fallback, attempt querying Supabase directly for the original answer sheet URL
      if ((!sourceDocPayload || isUsingDigitalBookletFallback) && exam.ExamId && submission.StudentId) {
        try {
          const subRes = await supabaseGetSubmissions(exam.ExamId, submission.StudentId);
          if (subRes.success && subRes.submissions && subRes.submissions.length > 0) {
            const matched = subRes.submissions.find((s) => s.StudentId === submission.StudentId);
            if (matched?.SubmissionUrl && !matched.SubmissionUrl.startsWith('stored://')) {
              sourceDocPayload = matched.SubmissionUrl;
            }
          }
        } catch (subFetchErr) {
          console.warn('Could not fetch latest submission from Supabase:', subFetchErr);
        }
      }

      // If absolutely no alternative exists, use loadedPdfBytes
      if (!sourceDocPayload) {
        sourceDocPayload = loadedPdfBytes;
      }
      ...
      // Persist graded evaluated booklet in durable storage
      if (gradedPdfDataUrl) {
        try {
          await saveGradedPaperStorage(exam.ExamId, submission.StudentId, gradedPdfDataUrl);
        } catch (storageErr) {
          console.warn('Could not cache graded paper in IndexedDB:', storageErr);
        }
      }
  ...
  const handleDownloadStudentBooklet = async () => {
    try {
      ...
      const sourcePayload = loadedPdfBytes || customPdfBase64 || submission.SubmissionUrl || '';
  ...
  // 11. Download Question-Wise Evaluation Report with EF Logo
  const handleDownloadQuestionWiseReport = async () => {
    try {
      setSaveSuccessToast('Generating Question-Wise Report with EF Logo...');
      const sessionData: OsmSessionData = {
        examId: exam.ExamId,
        studentId: submission.StudentId,
        studentName: submission.StudentName,
      ...
      const reportBlob = await generateQuestionWiseMarkingPdfBlob(sessionData);
      const blobUrl = URL.createObjectURL(reportBlob);
      const link = document.createElement('a');
      link.href = blobUrl;
      link.download = `ExamFriendly_Report_${exam.ExamId}_${submission.StudentId}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      setTimeout(() => URL.revokeObjectURL(blobUrl), 1000);
      setSaveSuccessToast('Question-Wise Report downloaded successfully!');
      setTimeout(() => setSaveSuccessToast(null), 3000);
    } catch (err: any) {
      console.error('Error generating question-wise report:', err);
      alert(`Could not download question-wise report: ${err.message || err}`);
    }
  };
      ...
      <div className="flex-1 flex min-h-0 relative overflow-hidden bg-slate-900">
        ...
        >
          ...
          {(fileSourceNotice || isUsingDigitalBookletFallback || (imagePages && imagePages.length > 0)) && !loadingPdf && (
            <div
              className={`w-full max-w-4xl rounded-xl p-3 px-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-lg shrink-0 ${
                isUsingDigitalBookletFallback
                  ? 'bg-amber-950/40 border border-amber-500/70 text-amber-200'
                  : 'bg-slate-900/95 border border-sky-800/80 text-sky-200'
              }`}
            >
              <div className="flex items-center gap-2.5 text-xs">
                ...
                <span className="leading-snug">
                  {isUsingDigitalBookletFallback
                    ? "Warning: Student answer booklet could not be retrieved from Drive (showing blank template). Upload student PDF/Photos or retry loading."
                    : fileSourceNotice}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0 flex-wrap">
                {isUsingDigitalBookletFallback && (
                  <button
                    type="button"
                    onClick={() => setLoadAttempt((p) => p + 1)}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold transition-all shadow-xs cursor-pointer"
                    title="Retry fetching student paper from Google Drive"
                  >
        ...
        <OsmEvaluationPanel
          ...
          onFinishChecking={handleFinishChecking}
          onSaveDraft={saveDraftToStorage}
          onDownloadQuestionWiseReport={handleDownloadQuestionWiseReport}
        />
      </div>
    </div>
  );
