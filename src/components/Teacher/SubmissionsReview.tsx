...
export const SubmissionsReview: React.FC<SubmissionsReviewProps> = ({ submissions, exams, onRefresh }) => {
  ...
  // Table Row Action Overflow Menu State
  const [openMenuSubKey, setOpenMenuSubKey] = useState<string | null>(null);

  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (!(e.target as HTMLElement).closest('.sub-action-menu-container')) {
        setOpenMenuSubKey(null);
      }
    };
  ...
  const handleOpenOsm = (sub: Submission) => {
    let matchedExam = exams.find((e) => e.ExamId.toUpperCase() === sub.ExamId.toUpperCase()) || exams.find((e) => e.ExamId === sub.ExamId);
    if (!matchedExam) {
      matchedExam = {
        ExamId: sub.ExamId,
    ...
    // Enrich with question marks bifurcation if missing
    if (!matchedExam.QuestionMarks || matchedExam.QuestionMarks.length === 0) {
      try {
        const storedQm = localStorage.getItem(`exam_qm_${sub.ExamId.toUpperCase()}`) || localStorage.getItem(`exam_qm_${sub.ExamId}`);
        if (storedQm) {
          const parsed = JSON.parse(storedQm);
          if (Array.isArray(parsed) && parsed.length > 0) {
            matchedExam = { ...matchedExam, QuestionMarks: parsed };
          }
        }
      } catch (e) {}
    }
  ...
  return (
    <div className="space-y-4 font-sans">
        ...
        <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg border border-slate-200 text-xs font-semibold">
          ...
          <button
            ...
            className={`px-3 py-1 rounded-md transition-colors flex items-center gap-1.5 cursor-pointer ${
              statusFilter === 'RECHECK' ? 'bg-amber-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            ...
            {pendingRechecksCount > 0 && (
              <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-mono ${
                statusFilter === 'RECHECK' ? 'bg-white text-amber-700 font-bold' : 'bg-amber-100 text-amber-800'
              }`}>
      ...
      {/* Submissions Table - De-boxed with 1px dividers and generous whitespace */}
      {filteredSubmissions.length === 0 ? (
        ...
        <div className="rounded-lg bg-white border border-slate-200 overflow-visible shadow-xs">
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-slate-100 text-slate-800">
                {filteredSubmissions.map((sub, idx) => {
                  ...
                  const rowKey = `${sub.ExamId}_${sub.StudentId}_${idx}`;

                  return (
                    <tr key={idx} className={`hover:bg-slate-50/70 transition-colors ${isRecheckRequested ? 'bg-amber-50/30' : ''}`}>
                      {/* 1. Candidate Name & ID - Left-aligned */}
                      <td className="px-5 py-3.5 text-left">
                        <div className="font-semibold text-slate-900">{sub.StudentName || sub.StudentId}</div>
                        <div className="text-[11px] font-mono text-slate-500">{sub.StudentId}</div>
                      </td>

                      {/* 2. Exam Code - Left-aligned */}
                      <td className="px-5 py-3.5 text-left font-mono font-semibold text-slate-800">
                        {sub.ExamId}
                      </td>

                      {/* 3. Submission Time - Right-aligned */}
                      <td className="px-5 py-3.5 text-right text-slate-500 font-mono text-[11px]">
                        {formatDateTime(sub.SubmittedAt)}
                      </td>

                      {/* 4. Score - Right-aligned as soft-tinted pill badge (e.g. 53 Marks in bg-emerald-50 text-emerald-700) */}
                      <td className="px-5 py-3.5 text-right">
                        {sub.Score !== '' && sub.Score !== undefined ? (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200/70 font-mono text-xs">
                            <Award className="w-3 h-3 text-emerald-600" />
                            <span>{sub.Score} Marks</span>
                          </span>
                        ) : (
                        ...
                        <div className="inline-flex flex-col items-center gap-1">
                          ...
                          {doubtsCount > 0 && (
                            ...
                            >
                              ...
                              <span>{doubtsCount} Query{doubtsCount > 1 ? 's' : ''}</span>
                            </button>
                          )}
                        </div>
                      </td>

                      {/* 6. Actions - Right-aligned: Primary Action Button + ⋮ Overflow Menu */}
                      <td className="px-5 py-3.5 text-right">
                        <div className="inline-flex items-center gap-2 justify-end">
                          ...
                          {isChecked ? (
                            <button
                              ...
                              onClick={() => {
                                ...
                                setPreviewExamId(sub.ExamId);
                                setPreviewStudentId(sub.StudentId);
                                setPreviewPdfTitle(`Checked Paper - ${sub.StudentId} (${sub.ExamId})`);
                              }}
                              className="px-3.5 py-1.5 text-xs font-semibold rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs transition-colors inline-flex items-center gap-1.5 min-h-[34px] cursor-pointer"
                            >
                            ...
                            <button
                              ...
                              className="px-3.5 py-1.5 text-xs font-semibold rounded-lg bg-amber-600 hover:bg-amber-700 text-white shadow-xs transition-colors inline-flex items-center gap-1.5 min-h-[34px] cursor-pointer"
                            ...
                            <button
                              type="button"
                              onClick={() => handleOpenOsm(sub)}
                              className="px-3.5 py-1.5 text-xs font-semibold rounded-lg bg-blue-600 hover:bg-blue-700 text-white shadow-xs transition-colors inline-flex items-center gap-1.5 min-h-[34px] cursor-pointer"
                              title="Open Online Screen Marking (OSM) to annotate and grade"
                            >
                          ...
                          {/* Secondary Actions: Subtle ⋮ Overflow Menu */}
                          <div className="relative sub-action-menu-container">
                            <button
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation();
                                setOpenMenuSubKey(openMenuSubKey === rowKey ? null : rowKey);
                              }}
                              className="p-1.5 rounded-lg border border-slate-200 text-slate-500 hover:text-slate-800 hover:bg-slate-100 transition-colors cursor-pointer min-h-[34px] min-w-[34px] inline-flex items-center justify-center"
                              title="More options"
                            ...
                            {openMenuSubKey === rowKey && (
                              <div className="absolute right-0 top-full mt-1 w-52 bg-white rounded-lg border border-slate-200 shadow-lg py-1.5 z-30 text-left">
                                {(sub.SubmissionUrl || (sub.RawImages && sub.RawImages.length > 0) || (sub.ImageUrls && sub.ImageUrls.length > 0)) && (
                                  <>
                                    <button
                                      ...
                                      onClick={() => {
                                        setOpenMenuSubKey(null);
                                        setPreviewPdfUrl(sub.SubmissionUrl || null);
                                        setPreviewExamId(sub.ExamId);
                                        setPreviewStudentId(sub.StudentId);
                                        setPreviewPdfTitle(`Answer Booklet - ${sub.StudentId} (${sub.ExamId})`);
                                      }}
                                      className="w-full px-3 py-2 text-xs text-slate-700 hover:bg-slate-50 flex items-center gap-2 cursor-pointer"
                                    >
                                      <Eye className="w-3.5 h-3.5 text-slate-500" />
                                      <span>Inspect Original Booklet</span>
                                    </button>
                                ...
                                {isChecked && (
                                  ...
                                  >
                                    ...
                                    <span>Edit Annotations (OSM)</span>
                                  </button>
                                )}
                                ...
                                >
                                  ...
                                  <span>Upload Offline Graded PDF</span>
                                </button>

                                {(isRecheckRequested || doubtsCount > 0) && (
                                  <button
                                    type="button"
                                    onClick={() => {
                                      setOpenMenuSubKey(null);
                                      handleOpenRecheck(sub);
                                    }}
                                    className="w-full px-3 py-2 text-xs text-amber-700 hover:bg-amber-50 flex items-center gap-2 border-t border-slate-100 cursor-pointer"
                                  >
                                    <RotateCcw className="w-3.5 h-3.5 text-amber-600" />
                                    <span>Resolve Recheck / Queries</span>
                                  </button>
                                )}
                              </div>
                            )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {gradingSubmission && (
                    <button
                      type="button"
                      onClick={() => {
                        setPreviewPdfUrl(gradingSubmission.SubmissionUrl || null);
                        setPreviewExamId(gradingSubmission.ExamId);
                        setPreviewStudentId(gradingSubmission.StudentId);
      ...
      {(previewPdfUrl || previewExamId) && (
        <PdfViewerModal
          isOpen={!!(previewPdfUrl || previewExamId)}
