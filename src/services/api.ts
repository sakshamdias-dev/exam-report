...
import { saveSubmissionFileStorage, getSubmissionFileStorageSync, getSubmissionFileStorage, saveQuestionPaperStorage, getQuestionPaperStorage } from '../utils/fileStorage';
import { fetchDrivePdfBinary } from '../utils/pdfHelper';
import { getSupabaseConfig, getSupabaseClient } from './supabaseClient';
import {
  supabaseUserLogin,
  supabaseGetStudents,
  supabaseCreateStudent,
  ...
  supabaseUpdateExam,
  supabaseDeleteExam,
  supabaseToggleExamResultsRelease,
  supabaseGetSubmissions,
  supabaseSaveSubmission,
  supabaseUploadGradedAnswerSheet,
  supabaseRequestRecheck,
  supabaseResolveRecheck,
  supabaseGetDoubts,
  supabaseCreateDoubt,
  supabaseAnswerDoubt,
  supabaseLogProctorEvent,
  supabaseGetProctorLogs,
  supabaseGetSubmissionOverrides,
  supabaseSetSubmissionOverride,
  supabaseDeleteSubmissionOverride,
} from './supabaseDb';
...
export function getGasConnectionStatus(): {
  ...
  const hasUrl = Boolean(cfg.webAppUrl && cfg.webAppUrl.trim().length > 0);
  return {
    mode: 'live',
...
// Helper to get configuration - reads stored custom URL, environment variable, or default
export function getGasConfig(): { webAppUrl: string; mode: 'live' } {
  if (typeof window !== 'undefined') {
    try {
      const stored = localStorage.getItem(STORAGE_KEYS.GAS_CONFIG);
      if (stored) {
        const parsed = JSON.parse(stored);
        if (parsed && typeof parsed.webAppUrl === 'string') {
          return { webAppUrl: cleanGasUrl(parsed.webAppUrl), mode: 'live' };
        }
      }
    } catch {}
  }
  const envUrl = (import.meta.env.VITE_GAS_WEB_APP_URL as string | undefined)?.trim();
  if (envUrl) {
    return { webAppUrl: cleanGasUrl(envUrl), mode: 'live' };
  }
  return { webAppUrl: cleanGasUrl(DEFAULT_WEB_APP_URL), mode: 'live' };
}

export function saveGasConfig(config: { webAppUrl: string; mode?: 'live' | 'simulator' }) {
  const cleanedConfig = {
    webAppUrl: cleanGasUrl(config.webAppUrl || ''),
    mode: 'live' as const,
  };
  if (typeof window !== 'undefined') {
...
export function clearGasConfig() {
  if (typeof window !== 'undefined') {
    localStorage.removeItem(STORAGE_KEYS.GAS_CONFIG);
  }
  gasLastErrorStatus = null;
  gasLastErrorMsg = null;
  gasLogged404Count = 0;
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('gas-config-updated', { detail: { webAppUrl: '', mode: 'live' } }));
  }
}

/**
 * Diagnostic test for Google Apps Script Web App connection
 */
export async function testGasConnection(urlToTest?: string): Promise<{
  ...
  const targetUrl = cleanGasUrl(urlToTest || getGasConfig().webAppUrl);
  if (!targetUrl) {
    return {
      success: false,
      error: 'Google Apps Script Web App URL is empty. Please enter your deployment URL.',
    };
  }

  try {
    const res = await sendGasRequestWithProgress(targetUrl, { action: 'ping' });
    gasLastErrorStatus = 200;
    gasLastErrorMsg = null;
    return {
      success: true,
      status: 200,
      message: res?.message || 'Google Apps Script backend and Google Drive PDF storage are active and ready!',
    };
  } catch (err: any) {
    const errMsg = String(err?.message || err);
    if (errMsg.includes('404')) {
      gasLastErrorStatus = 404;
      gasLastErrorMsg = 'Google Apps Script deployment returned HTTP 404. Ensure you deployed as Web App with "Who has access: Anyone".';
      return {
        success: false,
        status: 404,
        is404: true,
        error: 'HTTP 404: Deployment not found. Please ensure you clicked "Deploy > New deployment", selected "Web app", set "Execute as: Me", and "Who has access: Anyone".',
      };
    }
    return {
      success: false,
      error: errMsg || 'Failed to reach Google Apps Script Web App.',
    };
  }
}
...
function saveLocalExams(exams: Exam[]) {
  try {
    ...
    try {
      // In case quota is exceeded by large base64 data URIs, store with trimmed QPUrl for local cache
      const lightweight = exams.map((e) => ({
        ...e,
        QPUrl: e.QPUrl && e.QPUrl.startsWith('data:') && e.QPUrl.length > 500 ? '' : e.QPUrl,
      }));
      localStorage.setItem(STORAGE_KEYS.EXAMS, JSON.stringify(lightweight));
    } catch (err) {
      console.warn('Could not save exams to localStorage:', err);
    }
  }
}
...
/**
 * Dispatches database operations directly to Supabase while keeping file storage on Google Drive
 */
async function executeSupabaseAction<T = any>(
  action: string,
  ...
  try {
    switch (action) {
      case 'userLogin': {
        ...
        const res = await supabaseUserLogin(userId, password, role);
        if (res.success && res.user) {
          return {
            success: true,
            message: 'Signed in via Supabase PostgreSQL authentication.',
            data: { user: res.user } as any,
          };
        }
        return {
          success: false,
          error: res.error || 'Invalid credentials or user not found in Supabase.',
        };
      }

      case 'getStudents': {
        const res = await supabaseGetStudents();
        if (res.success) {
          return {
            success: true,
            data: { students: res.students } as any,
          };
        }
      ...
      case 'createStudent': {
        ...
        const password = payload.password || payload.Password || '';
        const res = await supabaseCreateStudent(studentId, name, password);
        if (res.success) {
          return {
            success: true,
            message: 'Student registered in Supabase database.',
            data: { student: res.student } as any,
          };
        }
        return { success: false, error: res.error };
      }
      ...
      case 'deleteStudent': {
        const studentId = payload.studentId || payload.UserId;
        const res = await supabaseDeleteStudent(studentId);
        return res.success
          ? { success: true, message: 'Student removed from Supabase.' }
          : { success: false, error: res.error };
      }

      case 'getGroups': {
        const res = await supabaseGetGroups();
        return res.success
          ? { success: true, data: { groups: res.groups } as any }
          : { success: false, error: res.error };
      }

      case 'createGroup': {
        const group = payload.group || payload;
        const res = await supabaseCreateGroup(group);
        return res.success
          ? { success: true, message: 'Student group created in Supabase.', data: { group: res.group } as any }
          : { success: false, error: res.error };
      }

      case 'updateGroup': {
        const group = payload.group || payload;
        const res = await supabaseUpdateGroup(group);
        return res.success
          ? { success: true, message: 'Group updated in Supabase.', data: { group: res.group } as any }
          : { success: false, error: res.error };
      }

      case 'deleteGroup': {
        const groupId = payload.groupId || payload.GroupId;
        const res = await supabaseDeleteGroup(groupId);
        return res.success
          ? { success: true, message: 'Group removed from Supabase.' }
          : { success: false, error: res.error };
      }

      case 'getAllExams': {
        const res = await supabaseGetAllExams();
        return res.success
          ? { success: true, data: { exams: res.exams } as any }
          : { success: false, error: res.error };
      }

      case 'createExam': {
        const rawFileBase64 = payload.qpFileBase64 || payload.pdfBase64 || payload.fileBase64 || '';
        const cleanExamId = (payload.examId || payload.ExamId || '').trim().toUpperCase() || `EXAM-${Date.now().toString(36).toUpperCase()}`;
        let qpUrl = payload.qpUrl || payload.QPUrl || '';

        // 1. Try Supabase Storage if configured
        const supabase = getSupabaseClient();
        if (!qpUrl && rawFileBase64 && supabase?.storage) {
          try {
            const cleanB64 = rawFileBase64.includes(';base64,') ? rawFileBase64.split(';base64,')[1] : rawFileBase64;
            ...
            const fileName = `${cleanExamId}_QP.pdf`;

            for (const bucket of ['question_papers', 'question-papers', 'exam-papers', 'exams', 'public']) {
              try {
            ...
            console.warn('[Supabase Storage] Question Paper upload attempt bypassed:', storageErr);
          }
        }

        // 2. Upload to Google Drive if Google Apps Script is configured
        if (!qpUrl && rawFileBase64 && gasConfig && gasConfig.webAppUrl) {
          if (gasLastErrorStatus === 404) {
            console.info('[File Storage] Google Drive Web App returned 404 previously. Safely storing Question Paper in Supabase database & local storage.');
          } else {
            try {
              onProgress?.({
                loaded: 30,
                ...
                percent: 30,
                stage: 'uploading',
                statusText: 'Syncing question paper to institutional Google Drive...',
              });
              const driveRes = await sendGasRequestWithProgress(gasConfig.webAppUrl.trim(), {
                action: 'uploadFileToDrive',
                fileBase64: rawFileBase64,
                filename: `QP_${cleanExamId}.pdf`,
                folder: 'Question_Papers',
                examId: cleanExamId,
              });
              if (driveRes && (driveRes.previewUrl || driveRes.fileUrl || driveRes.data?.previewUrl || driveRes.data?.fileUrl)) {
                qpUrl = driveRes.previewUrl || driveRes.fileUrl || driveRes.data?.previewUrl || driveRes.data?.fileUrl;
              }
            } catch (driveErr: any) {
              const errMsg = String(driveErr?.message || driveErr);
              if (errMsg.includes('404')) {
                gasLastErrorStatus = 404;
                gasLastErrorMsg = 'Google Apps Script deployment returned HTTP 404.';
              }
              console.info(`[File Storage] Google Drive sync bypassed (${errMsg.includes('404') ? 'Deployment 404' : 'Offline'}). Question paper archived safely in local storage & Supabase database.`);
            }
          }
        }

        // 3. Resilient fallback: Data URI
        if (!qpUrl && rawFileBase64) {
          qpUrl = rawFileBase64.startsWith('data:')
            ? rawFileBase64
            : `data:application/pdf;base64,${rawFileBase64}`;
        }

        // 4. Always save in client-side IndexedDB for instant offline-safe retrieval
        if (rawFileBase64) {
          try {
            await saveQuestionPaperStorage(cleanExamId, rawFileBase64);
          } catch (e) {
            console.warn('Could not save QP to IndexedDB:', e);
          }
        }

        // 5. Ensure StartTime, EndTime, and all fields are normalized and not null
        const startTime = payload.startTime || payload.StartTime || new Date().toISOString();
        const endTime = payload.endTime || payload.EndTime || new Date(Date.now() + 3 * 3600000).toISOString();
        const subject = payload.subject || payload.Subject || 'General Examination';
        const totalMarks = Number(payload.totalMarks ?? payload.TotalMarks ?? 100) || 100;

        const examToSave: Partial<Exam> = {
          ...payload,
          ExamId: cleanExamId,
          Subject: subject,
        ...
        const res = await supabaseCreateExam(examToSave);
        if (res.success && res.exam) {
          const exams = getLocalExams();
          const existingIdx = exams.findIndex((e) => e.ExamId.toUpperCase() === cleanExamId);
          if (existingIdx > -1) {
            exams[existingIdx] = res.exam;
          } else {
            exams.unshift(res.exam);
          }
      ...
      case 'updateExam': {
        const examId = (payload.examId || payload.ExamId || '').trim().toUpperCase();
        const res = await supabaseUpdateExam({ ...payload, ExamId: examId } as any);
        if (res.success && res.exam) {
          const exams = getLocalExams();
          const existingIdx = exams.findIndex((e) => e.ExamId.toUpperCase() === examId);
          if (existingIdx > -1) {
            exams[existingIdx] = res.exam;
            saveLocalExams(exams);
          }
          return { success: true, message: 'Exam updated in Supabase.', data: { exam: res.exam } as any };
        }
        return { success: false, error: res.error };
      }
      ...
      case 'deleteExam': {
        ...
        const res = await supabaseDeleteExam(examId);
        return res.success
          ? { success: true, message: 'Exam deleted from Supabase.' }
          : { success: false, error: res.error };
      }

      case 'toggleExamResultsRelease': {
        const res = await supabaseToggleExamResultsRelease(payload.examId, payload.resultsReleased);
        return res.success
          ? { success: true, message: 'Results release state updated in Supabase.', data: { exam: res.exam } as any }
          : { success: false, error: res.error };
      }

      case 'submitAnswerSheet':
      case 'uploadSubmissionPdf': {
        // Core migration requirement: Database in Supabase, File Storage in Google Drive!
        let driveUrl = payload.submissionUrl || payload.SubmissionUrl || '';
        if (gasConfig && gasConfig.webAppUrl && payload.pdfBase64) {
          if (gasLastErrorStatus === 404) {
            console.info('[File Storage] Google Drive Web App is currently 404. Archiving answer sheet safely in Supabase & local storage.');
          } else {
            try {
              onProgress?.({
                loaded: 25,
                ...
                percent: 25,
                stage: 'uploading',
                statusText: 'Streaming answer booklet to institutional Google Drive...',
              });
              const driveRes = await sendGasRequestWithProgress(gasConfig.webAppUrl.trim(), {
                action: 'submitAnswerSheet',
                studentId: payload.studentId,
                examId: payload.examId,
                pdfBase64: payload.pdfBase64,
                studentName: payload.studentName,
              }, onProgress);
              if (driveRes && (driveRes.previewUrl || driveRes.fileUrl || driveRes.data?.fileUrl || driveRes.data?.previewUrl)) {
                driveUrl = driveRes.previewUrl || driveRes.fileUrl || driveRes.data?.previewUrl || driveRes.data?.fileUrl || driveUrl;
              }
            } catch (driveErr: any) {
              const errMsg = String(driveErr?.message || driveErr);
              if (errMsg.includes('404')) {
              ...
              console.info(`[File Storage] Google Drive sync bypassed (${errMsg.includes('404') ? 'Deployment 404' : 'Offline'}). Answer sheet archived safely in local storage & Supabase database.`);
            }
          }
        }

        if (!driveUrl && payload.pdfBase64) {
          driveUrl = payload.pdfBase64.startsWith('data:')
            ? payload.pdfBase64
            : `data:application/pdf;base64,${payload.pdfBase64}`;
        }

        // Cache in browser IndexedDB
        saveSubmissionFileStorage(
          payload.examId,
          payload.studentId,
          payload.pdfBase64 || driveUrl,
          payload.rawImages || payload.imageUrls
        );

        // Store structured submission in Supabase
        const subRes = await supabaseSaveSubmission({
          StudentId: payload.studentId,
          ExamId: payload.examId,
          SubmissionUrl: driveUrl, // Google Drive link!
        ...
        supabaseLogProctorEvent({
          ...
          details: 'Answer booklet securely archived to Google Drive and registered in Supabase.',
        });
        ...
        if (subRes.success) {
          onProgress?.({
            ...
            percent: 100,
            stage: 'completed',
            statusText: 'Answer booklet submitted and verified in Supabase & Google Drive!',
          });
          return {
            success: true,
            message: 'Answer sheet saved to Google Drive and recorded in Supabase database.',
            data: {
              submission: subRes.submission,
              fileUrl: driveUrl,
              previewUrl: driveUrl,
            } as any,
          };
        }
        return { success: false, error: subRes.error };
      }

      case 'getSubmissions': {
        const res = await supabaseGetSubmissions(payload.examId, payload.studentId);
        return res.success
          ? { success: true, data: { submissions: res.submissions } as any }
          : { success: false, error: res.error };
      }

      case 'uploadGradedAnswerSheet': {
        // Upload evaluated PDF to Google Drive
        let gradedUrl = payload.gradedUrl || '';
        if (gasConfig && gasConfig.webAppUrl && payload.gradedPdfBase64) {
          if (gasLastErrorStatus === 404) {
            console.info('[File Storage] Google Drive Web App is currently 404. Archiving graded booklet safely in Supabase & local storage.');
          } else {
            try {
              const driveRes = await sendGasRequestWithProgress(gasConfig.webAppUrl.trim(), {
                action: 'uploadGradedAnswerSheet',
                studentId: payload.studentId,
                examId: payload.examId,
                gradedPdfBase64: payload.gradedPdfBase64,
                score: payload.score,
                feedback: payload.feedback,
              });
              if (driveRes && (driveRes.gradedUrl || driveRes.previewUrl || driveRes.data?.gradedUrl)) {
                gradedUrl = driveRes.previewUrl || driveRes.gradedUrl || driveRes.data?.previewUrl || driveRes.data?.gradedUrl || gradedUrl;
              }
            } catch (driveErr: any) {
              const errMsg = String(driveErr?.message || driveErr);
              if (errMsg.includes('404')) {
              ...
              console.info(`[File Storage] Google Drive sync bypassed (${errMsg.includes('404') ? 'Deployment 404' : 'Offline'}). Graded booklet archived safely in Supabase database.`);
            }
          }
        }

        if (!gradedUrl && payload.gradedPdfBase64) {
        ...
        const res = await supabaseUploadGradedAnswerSheet({
          studentId: payload.studentId,
          examId: payload.examId,
          gradedUrl,
          score: payload.score,
          ...
          osmDraftData: payload.osmDraftData,
        ...
        return res.success
          ? {
              success: true,
              message: 'Graded answer sheet saved to Google Drive and recorded in Supabase.',
              data: { submission: res.submission, gradedUrl } as any,
            }
          : { success: false, error: res.error };
      }

      case 'requestRecheck': {
        const res = await supabaseRequestRecheck(payload as any);
        return res.success
          ? { success: true, message: 'Recheck request submitted to Supabase.', data: { submission: res.submission } as any }
          : { success: false, error: res.error };
      }
      ...
      case 'resolveRecheck': {
        ...
        if (gasConfig && gasConfig.webAppUrl && payload.gradedPdfBase64) {
          if (gasLastErrorStatus === 404) {
            console.info('[File Storage] Google Drive Web App is currently 404. Archiving recheck result in Supabase.');
          } else {
            try {
              const driveRes = await sendGasRequestWithProgress(gasConfig.webAppUrl.trim(), {
                action: 'uploadGradedAnswerSheet',
              ...
              if (driveRes && (driveRes.gradedUrl || driveRes.previewUrl || driveRes.data?.gradedUrl)) {
                gradedUrl = driveRes.previewUrl || driveRes.gradedUrl || driveRes.data?.previewUrl || driveRes.data?.gradedUrl;
              }
            } catch (driveErr: any) {
              const errMsg = String(driveErr?.message || driveErr);
              if (errMsg.includes('404')) {
              ...
              console.info(`[File Storage] Google Drive sync bypassed (${errMsg.includes('404') ? 'Deployment 404' : 'Offline'}). Recheck archived safely in Supabase database.`);
            }
          }
        }
        const res = await supabaseResolveRecheck({ ...payload, gradedUrl } as any);
        return res.success
          ? { success: true, message: 'Recheck resolved in Supabase.', data: { submission: res.submission } as any }
          : { success: false, error: res.error };
      }

      case 'getDoubts': {
        const res = await supabaseGetDoubts(payload.examId);
        return res.success
          ? { success: true, data: { doubts: res.doubts } as any }
          : { success: false, error: res.error };
      }

      case 'createDoubt':
      case 'createEvaluationDoubt': {
        const res = await supabaseCreateDoubt(payload.doubt || payload);
        return res.success
          ? { success: true, message: 'Doubt recorded in Supabase.', data: { doubt: res.doubt } as any }
          : { success: false, error: res.error };
      }

      case 'answerDoubt':
      case 'answerEvaluationDoubt': {
        const res = await supabaseAnswerDoubt(payload.doubtId, payload.answer);
        return res.success
          ? { success: true, message: 'Doubt answered in Supabase.', data: { doubt: res.doubt } as any }
          : { success: false, error: res.error };
      }

      case 'logProctorEvent': {
        await supabaseLogProctorEvent(payload as any);
        return { success: true };
      }

      case 'getProctorLogs': {
        const res = await supabaseGetProctorLogs(payload.examId, payload.studentId);
        return res.success
          ? { success: true, data: { logs: res.logs } as any }
          : { success: false, error: res.error };
      }

      case 'getSubmissionOverrides': {
        const res = await supabaseGetSubmissionOverrides(payload.examId);
        return res.success
          ? { success: true, data: { overrides: res.overrides } as any }
          : { success: false, error: res.error };
      }

      case 'setSubmissionOverride':
      case 'saveSubmissionOverride': {
        const res = await supabaseSetSubmissionOverride(payload.examId, payload.override);
        return res.success
          ? { success: true, message: 'Override saved in Supabase.', data: { overrides: res.overrides } as any }
          : { success: false, error: res.error };
      }

      case 'deleteSubmissionOverride': {
        const res = await supabaseDeleteSubmissionOverride(payload.examId, payload.overrideId);
        return res.success
          ? { success: true, message: 'Override removed in Supabase.', data: { overrides: res.overrides } as any }
          : { success: false, error: res.error };
      }

      default:
        // Other actions (like getFileBase64) delegate to Google Apps Script / Google Drive
        return null;
    }
  } catch (err: any) {
    console.error(`[Supabase Action '${action}' Failed]:`, err);
    return null;
  }
}
...
export async function executeGasAction<T = any>(
  ...
  // 1. SUPABASE DATABASE ROUTING (When configured)
  // Relational operations execute against Supabase, while file storage remains on Google Drive!
  const supabaseConfig = getSupabaseConfig();
  if (supabaseConfig.isConfigured) {
    const supabaseResult = await executeSupabaseAction<T>(action, payload, onProgress, config);
  ...
  try {
    switch (action) {
      ...
      case 'getFileBase64': {
        ...
        if (examId && studentId) {
          let stored = getSubmissionFileStorageSync(examId, studentId);
          if (!stored?.rawImages?.length && !stored?.pdfBase64) {
            try {
              stored = await getSubmissionFileStorage(examId, studentId);
            } catch (_) {}
          }
