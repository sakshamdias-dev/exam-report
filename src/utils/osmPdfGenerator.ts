...
import { getEmbeddablePdfUrl, fetchDrivePdfBinary, isPdfOrImageBytes, uint8ArrayToBase64, resolvePdfBytesAndUrl, base64ToUint8Array } from './pdfHelper';
 ...
 * Automatically handles standard & progressive JPEG (SOF2), PNG, WebP, AVIF, HEIC, and Drive links.
 */
export async function embedImageSafe(pdfDoc: PDFDocument, imgSrc: string): Promise<PDFImage | null> {
  ...
  let cleanData = imgSrc.trim();
  if (!cleanData) return null;

  // Resolve stored:// references from IndexedDB
  if (cleanData.startsWith('stored://')) {
    try {
      const resolved = await resolvePdfBytesAndUrl(cleanData);
      if (resolved.imageSrc) {
        cleanData = resolved.imageSrc;
      } else if (resolved.dataUri) {
        cleanData = resolved.dataUri;
      }
    } catch (_) {}
  }

  // Helper to convert any image source to compliant PNG bytes via offscreen canvas
  const convertViaCanvas = (src: string): Promise<Uint8Array | null> => {
    return new Promise((resolve) => {
      ...
      // Safety timeout: 7 seconds maximum to prevent hanging
      ...
      try {
        ...
        const processImg = async () => {
          try {
            ...
            let h = img.naturalHeight || img.height || 1100;
            if (w <= 0) w = 800;
            if (h <= 0) h = 1100;

            // Scale down gigantic smartphone camera images to maximum 2048px dimension
            const MAX_DIM = 2048;
            ...
            // Direct synchronous PNG extraction ensures 100% reliability across all environments
            const pngData = canvas.toDataURL('image/png');
            clearTimeout(timer);
            const parts = pngData.split('base64,');
            if (parts.length < 2) return finish(null);
        ...
        if (!src.startsWith('http://') && !src.startsWith('https://') && !src.startsWith('blob:') && !src.startsWith('data:')) {
          ...
          formattedSrc = `data:${isPng ? 'image/png' : 'image/jpeg'};base64,${src.replace(/\s/g, '')}`;
        }
  ...
  if (cleanData.includes('base64,') || cleanData.startsWith('data:image/') || (!cleanData.startsWith('http') && cleanData.length > 100)) {
    try {
      ...
      try {
        ...
        // pdf-lib's embedJpg fails on progressive JPEGs (SOF2 marker 0xc2), WebP, etc.
        // Fall back directly to canvas conversion which decodes any JPEG/WebP/HEIC reliably
        try {
      ...
      console.warn('Direct base64 decode for image embed failed, falling back to canvas:', decodeErr);
    }
  }
...
export async function generateCheckedPdfBlob(
  ...
  try {
    ...
    let storedPdfBase64: string | null = null;

    // STEP 1: Search IndexedDB storage & local cache for student rawImages or saved pdfBase64
    if (sessionData.examId && sessionData.studentId) {
      try {
        const stored = await getSubmissionFileStorage(sessionData.examId, sessionData.studentId);
        if (stored?.rawImages && stored.rawImages.length > 0 && effectiveImages.length === 0) {
          effectiveImages = [...stored.rawImages];
        }
        if (stored?.pdfBase64) {
          storedPdfBase64 = stored.pdfBase64;
          try {
            const rawB64 = stored.pdfBase64.includes('base64,')
              ? stored.pdfBase64.split('base64,')[1]
              : stored.pdfBase64;
            const binStr = atob(rawB64.replace(/\s/g, ''));
            const u8 = new Uint8Array(binStr.length);
            for (let k = 0; k < binStr.length; k++) u8[k] = binStr.charCodeAt(k);
            if (isValidPdfBytes(u8)) {
              originalBytes = u8;
            }
          } catch (_) {}

          if (!originalPdfUrlOrData || (typeof originalPdfUrlOrData === 'string' && (originalPdfUrlOrData.startsWith('http') || originalPdfUrlOrData.startsWith('stored://')))) {
            originalPdfUrlOrData = stored.pdfBase64;
          }
        }
      } catch (e) {
        console.warn('Error reading stored files in generateCheckedPdfBlob:', e);
      }

      if (effectiveImages.length === 0) {
        for (const k of [
          `osm_student_images_${sessionData.examId}_${sessionData.studentId}`,
          `osm_student_images_${sessionData.examId.toUpperCase()}_${sessionData.studentId.toUpperCase()}`,
        ]) {
          try {
    ...
    // STEP 2: Handle stored:// URI resolution if passed as originalPdfUrlOrData
    if (typeof originalPdfUrlOrData === 'string' && originalPdfUrlOrData.trim().startsWith('stored://')) {
      try {
        const resolved = await resolvePdfBytesAndUrl(originalPdfUrlOrData, {
          examId: sessionData.examId,
          studentId: sessionData.studentId,
        });
        if (resolved.imagePages && resolved.imagePages.length > 0 && effectiveImages.length === 0) {
          effectiveImages = [...resolved.imagePages];
        } else if (resolved.imageSrc && effectiveImages.length === 0) {
          effectiveImages = [resolved.imageSrc];
        } else if (resolved.bytes && isValidPdfBytes(resolved.bytes)) {
          originalBytes = resolved.bytes;
        } else if (resolved.dataUri) {
          originalPdfUrlOrData = resolved.dataUri;
        }
      } catch (storedErr) {
        console.warn('Error resolving stored:// in generateCheckedPdfBlob:', storedErr);
      }
    }

    // STEP 3: If effectiveImages is still empty, check if originalPdfUrlOrData is an image or contains images
      ...
      if (trimmed.startsWith('data:image/')) {
        ...
        if (pdfInfo.isDriveUrl && (pdfInfo.driveFileId || trimmed)) {
          try {
            ...
            } else if (driveBin && driveBin.bytes && isValidPdfBytes(driveBin.bytes)) {
              originalBytes = driveBin.bytes;
            }
          } catch (_) {}
        }
      }
    ...
    // STEP 4: If student submitted photos / images, embed each photo onto its corresponding page
    if (effectiveImages.length > 0) {
      ...
      for (let i = 0; i < effectiveImages.length; i++) {
        ...
        if (imgDataUrl) {
          try {
            ...
            if (embeddedImg) {
              const pageW = (dim?.width && dim.width > 50) ? dim.width : (embeddedImg.width || 595.28);
              const pageH = (dim?.height && dim.height > 50) ? dim.height : (embeddedImg.height || 841.89);
              ...
              console.warn(`embedImageSafe returned null on page ${i + 1}; drawing standard page`);
          ...
          if (trimmed.startsWith('data:application/pdf;base64,') || trimmed.startsWith('data:')) {
            ...
            if (pdfInfo.isDriveUrl && (pdfInfo.driveFileId || trimmed)) {
              try {
                ...
                if (gasRes.success) {
                  if (gasRes.data?.rawImages && gasRes.data.rawImages.length > 0 && effectiveImages.length === 0) {
                    effectiveImages = [...gasRes.data.rawImages];
                  } else if ((gasRes as any).rawImages && (gasRes as any).rawImages.length > 0 && effectiveImages.length === 0) {
                    effectiveImages = [...(gasRes as any).rawImages];
                  }
                  if (gasRes.data?.base64Data) {
              ...
              if (!originalBytes && pdfInfo.driveFileId) {
                try {
                  ...
                  if (driveBinary) {
                    if (driveBinary.isImage && driveBinary.base64Data && effectiveImages.length === 0) {
                      effectiveImages = [driveBinary.base64Data];
                    } else if (driveBinary.bytes && isValidPdfBytes(driveBinary.bytes)) {
      ...
      if (!originalBytes && (storedPdfBase64 || (sessionData.examId && sessionData.studentId))) {
        try {
          const rawB64 = storedPdfBase64 || (await getSubmissionFileStorage(sessionData.examId, sessionData.studentId))?.pdfBase64;
          if (rawB64) {
            const b64 = rawB64.includes('base64,') ? rawB64.split('base64,')[1] : rawB64;
            const binaryStr = atob(b64.replace(/\s/g, ''));
            const bytes = new Uint8Array(binaryStr.length);
            for (let i = 0; i < binaryStr.length; i++) {
              bytes[i] = binaryStr.charCodeAt(i);
      ...
      // CRITICAL CHECK: If originalBytes was NOT found, OR if originalBytes was the fallback digital booklet,
      // actively query Supabase for the student's authentic uploaded answer booklet!
      let isFallbackBooklet = false;
      if (originalBytes && originalBytes.length > 0) {
        try {
          const sampleText = new TextDecoder('utf-8', { fatal: false }).decode(originalBytes.slice(0, 3000));
          if (
            sampleText.includes('EXAMINATION ANSWER SCRIPT / EVALUATION BOOKLET') ||
            sampleText.includes('Official Digital Submission Record for On-Screen Marking')
          ) {
            isFallbackBooklet = true;
          }
        } catch (_) {}
      }

      if ((!originalBytes || isFallbackBooklet) && sessionData.examId && sessionData.studentId) {
        try {
          const subRes = await supabaseGetSubmissions(sessionData.examId, sessionData.studentId);
          if (subRes.success && subRes.submissions && subRes.submissions.length > 0) {
            const matched = subRes.submissions.find((s) => s.StudentId === sessionData.studentId);
            if (matched?.SubmissionUrl) {
              const trimmed = matched.SubmissionUrl.trim();
              if (
                trimmed.startsWith('data:application/pdf') ||
                trimmed.startsWith('data:') ||
                trimmed.startsWith('JVBERi0')
              ) {
                const base64Data = trimmed.includes('base64,') ? trimmed.split('base64,')[1] : trimmed;
                ...
                if (pdfInfo.isDriveUrl && (pdfInfo.driveFileId || trimmed)) {
                  const driveBin = await fetchDrivePdfBinary(pdfInfo.driveFileId || '', trimmed);
                  if (driveBin?.bytes && isValidPdfBytes(driveBin.bytes)) {
                    originalBytes = driveBin.bytes;
                  } else if (driveBin?.isImage && driveBin.base64Data) {
                    effectiveImages = [driveBin.base64Data];
                  }
                }
              }
        ...
        } catch (supabaseSubErr) {
          console.warn('Could not query Supabase for authentic student answer booklet:', supabaseSubErr);
        }
      }
      ...
      if (originalBytes && isValidPdfBytes(originalBytes)) {
        try {
          ...
          console.warn('Failed to load PDF document bytes directly with pdf-lib. Using PDF.js rasterizer fallback:', loadErr);
          pdfDoc = null;
        }
      }

      // High-fidelity fallback: if pdf-lib failed to load the PDF structure (e.g. mobile scanner compressions),
      // render each page of the student's actual PDF with pdfjsLib to canvas and embed as PNG images!
      if (!pdfDoc && originalBytes && isValidPdfBytes(originalBytes)) {
        try {
          ...
          for (let p = 1; p <= numPages; p++) {
            ...
            const dim = renderedPageDimensions[p - 1];
            // Render at 2.0x scale for crisp scan/handwriting reproduction
            ...
            if (ctx) {
              ...
              const bin = atob(parts[1].replace(/\s/g, ''));
              const pngBytes = new Uint8Array(bin.length);
              for (let k = 0; k < bin.length; k++) pngBytes[k] = bin.charCodeAt(k);

              const embeddedImg = await pdfDoc.embedPng(pngBytes);
              const targetW = (dim?.width && dim.width > 50) ? dim.width : (viewport.width / 2.0);
              const targetH = (dim?.height && dim.height > 50) ? dim.height : (viewport.height / 2.0);
          ...
          console.warn('PDF.js rasterizer fallback error in generateCheckedPdfBlob:', rasterErr);
          pdfDoc = null;
        }
      }

      // If document still could not be loaded, check if student submitted rawImages in storage before resorting to blank booklet
      if (!pdfDoc && sessionData.examId && sessionData.studentId) {
        try {
          const stored = await getSubmissionFileStorage(sessionData.examId, sessionData.studentId);
          if (stored?.rawImages && stored.rawImages.length > 0) {
            pdfDoc = await PDFDocument.create();
            for (let i = 0; i < stored.rawImages.length; i++) {
              const imgDataUrl = stored.rawImages[i];
              const dim = renderedPageDimensions[i];
              const embeddedImg = await embedImageSafe(pdfDoc, imgDataUrl);
              if (embeddedImg) {
                const pageW = (dim?.width && dim.width > 50) ? dim.width : (embeddedImg.width || 595.28);
      ...
      // If absolutely no document or student images could be found anywhere, build clean digital evaluation booklet
      if (!pdfDoc) {
        try {
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      if (hasAnnotations) {
        ...
        if (typeof document !== 'undefined') {
          // Render annotations to high-res canvas (2x) in browser
          ...
          if (canvas) {
            ...
            const len = binaryString.length;
            pngBytes = new Uint8Array(len);
            for (let j = 0; j < len; j++) {
              pngBytes[j] = binaryString.charCodeAt(j);
            }
          }
        } else {
          // Node environment fallback using sharp & SVG
          try {
            const sharpModule = await (new Function('m', 'return import(m)'))('sharp');
            const sharp = (sharpModule as any).default || sharpModule;
            const svgElements: string[] = [];

            if (pageAnnotations.strokes && pageAnnotations.strokes.length > 0) {
              const { getStroke } = await import('perfect-freehand');
              const { getSvgPathFromStroke } = await import('./strokeHelper');
              for (const s of pageAnnotations.strokes) {
                ...
                if (pathData) {
                  svgElements.push(`<path d="${pathData}" fill="${s.color || '#dc2626'}" opacity="${s.opacity ?? 1}" />`);
                }
              }
            }

            if (svgElements.length > 0) {
              const svg = `<svg width="${Math.round(pageW * 2)}" height="${Math.round(pageH * 2)}" viewBox="0 0 ${pageW} ${pageH}" xmlns="http://www.w3.org/2000/svg">${svgElements.join('')}</svg>`;
              const pngBuf = await sharp(Buffer.from(svg)).png().toBuffer();
              pngBytes = new Uint8Array(pngBuf);
            }
          } catch (nodeRenderErr) {
            console.warn('Node annotation render fallback notice:', nodeRenderErr);
          }
        }
...
export async function generateStudentBookletPdf(
  ...
  try {
    ...
    // Check 1.5: If originalPdfUrlOrData is a stored:// URI, resolve it directly
    if (typeof originalPdfUrlOrData === 'string' && originalPdfUrlOrData.trim().startsWith('stored://')) {
      try {
        const resolved = await resolvePdfBytesAndUrl(originalPdfUrlOrData, {
          examId: fallbackInfo?.examId,
          studentId: fallbackInfo?.studentId,
        });
        if (resolved.imagePages && resolved.imagePages.length > 0 && (!imagePages || imagePages.length === 0)) {
          imagePages = resolved.imagePages;
        } else if (resolved.bytes && isValidPdfBytes(resolved.bytes)) {
          return new Blob([resolved.bytes as any], { type: 'application/pdf' });
        } else if (resolved.dataUri) {
          originalPdfUrlOrData = resolved.dataUri;
        }
      } catch (_) {}
