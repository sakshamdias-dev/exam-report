import { getQuestionPaperStorage, getSubmissionFileStorage } from './fileStorage';
...
export async function resolvePdfBytesAndUrl(
  ...
  if (isStoredRef || !rawStr) {
    const cleanExamId = options?.examId || (rawStr.startsWith('QP_') ? rawStr.replace(/^QP_/i, '') : null);

    if (cleanExamId) {
      // Check student submission first if studentId provided
      if (options?.studentId) {
        try {
          const stored = await getSubmissionFileStorage(cleanExamId, options.studentId);
